...
(function(){
	 ...
	 * 
	 * @name Editor
	 * @desc 创建一个跟编辑器实例 - ***container*** 编辑器容器对象 - ***iframe*** 编辑区域所在的iframe对象 - ***window*** 编辑区域所在的window -
	 *       ***document*** 编辑区域所在的document对象 - ***body*** 编辑区域所在的body对象 - ***selection*** 编辑区域的选区对象
	 */
	...
	Editor.prototype = {
		/**
		 * 当编辑器ready后执行传入的fn,如果编辑器已经完成ready，就马上执行fn，fn的中的this是编辑器实例。 大部分的实例接口都需要放在该方法内部执行，否则在IE下可能会报错。
		 * 
		 * @name ready
		 * @grammar editor.ready(fn) fn是当编辑器渲染好后执行的function
		 * @example var editor = new UM.ui.Editor(); editor.render("myEditor"); editor.ready(function(){
		...
		sync : function(formId){
			var me = this, form = formId ? document.getElementById(formId) : domUtils.findParent(me.body.parentNode, function(node){
		 ...
		 * @example getContent默认是会现调用hasContents来判断编辑器是否为空，如果是，就直接返回空字符串 你也可以传入一个fn来接替hasContents的工作，定制判断的规则
		 *          editor.getContent(function(){ return false //编辑器没有内容 ，getContent直接返回空 })
		 */
		getContent : function(){
			var me = this;
			return UM.formulaParser(me.getContentTxt());
		},
		getHtml : function(cmd, fn, notSetCursor, ignoreBlank, formatter){
			var me = this;
		 ...
		 * @example var editor = new UM.ui.Editor() editor.ready(function(){ //需要ready后执行，否则可能报错
		 *          editor.setContent("欢迎使用UEditor！"); })
		 */
		 ...
		 * @example //禁用工具栏中除加粗和插入图片之外的所有功能 editor.disable(['bold','insertimage']);//可以是单一的String,也可以是Array
		 */
