...
shr.defineClass("shr.ats.FillSignCardList", shr.framework.List, {
	...
	initalizeDOM : function () {
		...
		jsBinder.gridLoadComplete=function(data){
			shr.ats.FillSignCardList.superClass.gridLoadComplete.call(this)
...
//			shr.ats.FillSignCardList.superClass.queryGrid.call(this);
	...
	,setGridCustomParam: function() {
		...
		if (atsCalFlag){
			...
			var fastFilterItems = this.getFastFilterItems();
			if(fastFilterItems && fastFilterItems['entries.attendDate'] && fastFilterItems['entries.attendDate'].values){
				params["beginDate"] = this.getFastFilterItems()['entries.attendDate'].values["startDate"];
				params["endDate"] = this.getFastFilterItems()['entries.attendDate'].values["endDate"];
			} else {
...
//				}
	...
	*/
	...
	,againstApproveAction : function () {
		...
		$('#orgFillDiv').dialog({
			title: jsBizMultLan.atsManager_fillSignCardList_i18n_21,
			width: 900,
			height: 400,
			modal: true,
			resizable: false,
			...
			buttons: [{
					text: jsBizMultLan.atsManager_fillSignCardList_i18n_19,
					click: function() {						
						$(this).dialog( "close" );
						...
						if (reason) {		
							...
							shr.showError({message: jsBizMultLan.atsManager_fillSignCardList_i18n_22});
							return;
						} 
					}
				},{
					text: jsBizMultLan.atsManager_fillSignCardList_i18n_20,
					click: function(){
						$(this).dialog( "close" );
					}
				}],
	...
	,onCellSelect: function(rowid, colIndex, cellcontent, e) {
		if (!atsCalFlag){
			shr.ats.FillSignCardList.superClass.onCellSelect.call(this,rowid, colIndex, cellcontent, e);
		}else {
			return true;	
		}	
	}
