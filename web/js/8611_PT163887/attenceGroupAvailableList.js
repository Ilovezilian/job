shr.defineClass("shr.ats.attenceGroupAvailableList", shr.ats.AtsAvailableBasicItemList, {
	initalizeDOM : function () {
		shr.ats.attenceGroupAvailableList.superClass.initalizeDOM.call(this);
		var that = this;
	},
	...
	shr.showWarning({message : jsBizMultLan.atsManager_attenceGroupAvailableList_i18n_0});
	return;
	}
	...
	var _self = this;
	$('#blindCurrent').attr('values','attenceGroup');
	$('#importDiv').remove();
	var importDiv = $('<div id="importDiv"></div>').appendTo($('body'));	
	importDiv.dialog({
	modal : false,
	title : jsBizMultLan.atsManager_attenceGroupAvailableList_i18n_3,
	width : 1035,
	minWidth : 1035,
	height : 505,
	minHeight : 505,
	...
	importDiv.dialog("close");
		...
		if(gridData.length<=0){
			shr.showWarning({message : jsBizMultLan.atsManager_attenceGroupAvailableList_i18n_2});
			return;
		}
		...
		openLoader(1,jsBizMultLan.atsManager_attenceGroupAvailableList_i18n_4);
		shr.ajax({
			type:"post",
			async:true,
			url:url,
		...
		shr.showWarning({message : jsBizMultLan.atsManager_attenceGroupAvailableList_i18n_1});
		return;
		}
		if (selectedIds.length > 0) {
		var bills;
