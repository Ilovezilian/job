...
shr.defineClass("shr.ats.AtsOverTimeBillAllList", shr.framework.List, {
	...
	initalizeDOM : function () {
		...
		shr.ats.AtsOverTimeBillAllList.superClass.initalizeDOM.call(this);
		jsBinder.gridLoadComplete=function(data){
			...
			shr.ats.AtsOverTimeBillAllList.superClass.gridLoadComplete.call(that,data);
	...
	queryGrid: function(){
		...
		if(dateRequiredValidate){
			shr.ats.AtsOverTimeBillAllList.superClass.queryGrid.call(this);
		}
	}
	...
	,setGridCustomParam: function() {
		...
		if (atsCalFlag){
			...
			if(fastFilterItems && fastFilterItems['entries.otDate'] && fastFilterItems['entries.otDate'].values){
				params["beginDate"] = this.getFastFilterItems()['entries.otDate'].values["startDate"];
				params["endDate"] = this.getFastFilterItems()['entries.otDate'].values["endDate"];
			} else {
	...
	,addNewAction: function() {
		this.reloadPage({
			uipk: "com.kingdee.eas.hr.ats.app.AtsOverTimeBillAllForm",
			method: 'addNew'
		});
	}
	,addNewBatchAction: function() {
		this.reloadPage({
			uipk: "com.kingdee.eas.hr.ats.app.AtsOverTimeBillAllBatchForm",
			method: 'addNew'
		});
	}
	/**
	...
	,doAgainstApproveAction: function(billId){
		...
		$('#orgFillDiv').dialog({
			title: jsBizMultLan.atsManager_atsOverTimeBillAllList_i18n_21,
			width: 900,
			height: 400,
			modal: true,
			resizable: false,
			...
			,
			buttons: [{
				text: jsBizMultLan.atsManager_atsOverTimeBillAllList_i18n_20,
				click: function() {						
					$(this).dialog( "close" );
					...
					if (reason) {
						...
						shr.showError({message: jsBizMultLan.atsManager_atsOverTimeBillAllList_i18n_22});
						return;
					} 
				}
			},{
				text: jsBizMultLan.atsManager_atsOverTimeBillAllList_i18n_19,
				click: function(){
					$(this).dialog( "close" );
				}
			}],
	...
	*/
	...
	,onCellSelect: function(rowid, colIndex, cellcontent, e) {
		if (!atsCalFlag){
			shr.ats.AtsOverTimeBillAllList.superClass.onCellSelect.call(this,rowid, colIndex, cellcontent, e);
		}else {
			return true;	
		}	
	}
