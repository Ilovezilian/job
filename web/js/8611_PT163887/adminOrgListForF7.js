shr.defineClass("shr.ats.adminOrgListForF7", shr.framework.List, {
	...
	,deletePositionAction: function(){
		...
		if (selectedId == undefined ) {
	        alert(jsBizMultLan.atsManager_adminOrgListForF7_i18n_3);
			return ;
	    }
		
		var _self = this;
		shr.showConfirm(jsBizMultLan.atsManager_adminOrgListForF7_i18n_1, function(){
	...
	,selectOkAction:function(){
	    ...
	    var iii=0;
	    var names=[];
	    for(iii=0;iii<selectedNames.length;iii++){
	    if(selectedNames[iii].ctnCld=="1"){
	     names.push(selectedNames[iii].name+jsBizMultLan.atsManager_adminOrgListForF7_i18n_0);
	    }else{
	     names.push(selectedNames[iii].name);
	    }
	    }
	    ...
	    if (selectedIds&&selectedIds.length>0) {
			...
			success : function (response) {
				var list = response.idList;
								var listString ="( ";	
								for(var i=0;i<list.length;i++){
								   listString =   listString + " '"+list[i] + "' ,";
								}
								listString =  listString.trim().substring(0,listString.length - 1) + ")";
								$('#adminOrg_el',winPrtDoc).val(listString);
								$('#adminOrg',winPrtDoc).val(names);
								$('#closeDialogFlag',winPrtDoc).attr("value","1");
								
								//$('#operationDialog-frame',winPrtDoc).dialog('close');
			}
		});
	...
	,getSelectedIds: function() {
		...
		if (selectedIds.length > 0) {
			...
			if(billIds.length==0){
			   shr.showWarning({
			     message: jsBizMultLan.atsManager_adminOrgListForF7_i18n_4
		       });
		       return ;
			}else{
				if(isSealUpCount>0){
				   shr.showWarning({
				     message: jsBizMultLan.atsManager_adminOrgListForF7_i18n_5
			       });
				}
				return   billIds;
			}
		...
		shr.showWarning({
			message: jsBizMultLan.atsManager_adminOrgListForF7_i18n_2
		});
	}
		/**
	 * 获得选中的ids,返回的是一个对象数组（除ID外还有其他信息）
	...
	,getSelectedNames: function() {
		var $grid = $(this.gridId);
		var selectedNames = $grid.jqGrid("getSelectedRows");
		if (selectedNames.length > 0) {
			var billNames = [];
			var isSealUpCount=0;
			for (var i = 0, length = selectedNames.length; i < length; i++) {
				var isSealUp=$grid.jqGrid("getCell", selectedNames[i], "isSealUp");
				if(isSealUp=='false'){//没封存，即启用
					var vid={};
					vid.id=$grid.jqGrid("getCell", selectedNames[i], "id");
					vid.name=$grid.jqGrid("getCell", selectedNames[i], "name");
					//alert($grid.jqGrid("getCell", selectedNames[i], "name"));
					//alert($grid.jqGrid("getCell", selectedNames[i], "containChildYN"));//containChildYN
					vid.ctnCld=$('tr[id="'+vid.id+'"] input[id^="jqg_grid_containChild_"]:checked').size();
					billNames.push(vid);
				}else{ 
					isSealUpCount++;
