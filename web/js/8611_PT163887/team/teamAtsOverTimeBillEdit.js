...
shr.defineClass("shr.ats.team.TeamAtsOverTimeBillEdit", shr.framework.Edit, {
	_uiClass:"",
	effectiveFlag:true,
	isOTControl:false,
    ...
    _uipk: {
		atsOverTimeBillForm : "com.kingdee.eas.hr.ats.app.AtsOverTimeBillForm",
		atsOverTimeBillList : 'com.kingdee.eas.hr.ats.app.team.AtsOverTimeBill.list',
    },
    handler: {
		teamAtsOverTimeBillEditHandler : "com.kingdee.shr.ats.web.handler.team.TeamAtsOverTimeBillEditHandler",
	},
	initalizeDOM:function(){
		shr.ats.team.TeamAtsOverTimeBillEdit.superClass.initalizeDOM.call(this);
		_globalObject = this;
		var that = this ;
...
//		判断【加班费】是否有效（存在且生效）
		...
		if (shr.getCurrentViewPage().uipk == _globalObject._uipk.atsOverTimeBillForm) {
...
//						$('#entries_otCompens').parent().next().css("display","none");
	...
	,getOTCompensByOTType : function ( personId, otTypeId) {
		var _self = this;
		var otCompens="";
		atsMlUtile.getFieldOriginalValue('entries_otDate')
		var otDate = atsMlUtile.getFieldOriginalValue('entries_otDate');
		if(otDate && otDate != '' && personId != '' && otTypeId != ''){
			_self.remoteCall({
				type:"post",
...
//	判断加班补偿方式是否有效（存在且生效）
	...
	,cancelAction:function(){
		...
		this.reloadPage({
			uipk: _globalObject._uipk.atsOverTimeBillList
		});
	}
	/**
	 * 专员列表点击取消的方法
	...
	,getOverTimeType:function(otDate,personId){
		var _self = this;
		var url = shr.getContextPath() + "/dynamic.do?handler=" + this.handler.teamAtsOverTimeBillEditHandler + "&method=getOverTimeType";
...
//	    	var otDate=$(this).val()||'';
		...
		if ( startTime!=""&&startTime!=null && endTime!=""&&endTime!=null ) {
			...
			startTime = startTime.replace(regEx,"/");
		 	...
		 	if (longTime <= 0) {
			 	...
			 	t1 = t1.toFixed(atsMlUtile.getSysDecimalPlace());
			 	if (t1 < 0) {
			 		atsMlUtile.setTransNumValue("entries_applyOTTime",0);
		 			//$("#entries_realOTTime").val(0); //时刻根据  实际的止-实际的起-休息通过在调用that.calculataApplyOTTime()后调用that.calculataRealOTTime()来达到的
			 	}else{
					atsMlUtile.setTransNumValue("entries_applyOTTime",t1);
		...
		if ( realStartTime!=""&&realStartTime!=null && realEndTime!=""&&realEndTime!=null ) {
			...
			realStartTime = realStartTime.replace(regEx,"/");
		 	...
		 	if (floatTime <= 0.0) {
			 	...
			 	//t1 = parseFloat(longTime)/1000.0/60/60;
			 	var  t1 = floatTime.toFixed(atsMlUtile.getSysDecimalPlace());
				atsMlUtile.setTransNumValue("entries_realOTTime",t1);
		 	}
		}
	},
	...
	calculataApplyRestTime : function(){
		...
		var url = shr.getContextPath() + "/dynamic.do?handler=" + this.handler.teamAtsOverTimeBillEditHandler + "&method=getMyRestTime";
...
//	},
    ...
    assembleModel: function() {
        var model = shr.ats.team.TeamAtsOverTimeBillEdit.superClass.assembleModel.call(this);
	...
	,getCurrentModel : function(){
		...
		var that = this ;
		var model = shr.ats.team.TeamAtsOverTimeBillEdit.superClass.getCurrentModel.call(this);
