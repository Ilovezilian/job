...
var hideButtonId = [];
var currentCalUrl = "com.kingdee.eas.hr.ats.app.team.AttenceResultSum.list";//记录考勤计算当前访问的URL
shr.defineClass("shr.ats.team.TeamAtsResultToSumList", shr.framework.List, {
	pageStep: 0,
	showAttendJson : null ,
	isFirstLoad: true,
	attendMap : [] ,
	...
	initalizeDOM : function () {
		...
		shr.ats.team.TeamAtsResultToSumList.superClass.initalizeDOM.call(_self);
...
//		_self.summaryorDetailHover();
	...
	getPageState: function(){
		...
		return shr.ats.team.TeamAtsResultToSumList.superClass.getPageState.call(this);
	},
	...
	queryGridByEvent: function(e){
		var uipkArr = ['com.kingdee.eas.hr.ats.app.team.AttenceResultSum.list','com.kingdee.eas.hr.ats.app.team.AttendanceResult.dynamicList', 'com.kingdee.eas.hr.ats.app.team.AttendanceResultSum.dynamicList', 'com.kingdee.eas.hr.ats.app.team.AttendanceResultSalary.dynamicList'];
		//处理快速过滤
		setTimeout(function(){
 	...
 	onNaviLoad : function() {
 		...
 		$('#switchDetail').parent().click(function(){ // 已计算人员(明细展示)
			if($("#switchDetail").html()==jsBizMultLan.atsManager_atsResultToSumList_i18n_21){
				...
				that.reloadPage({
					uipk: 'com.kingdee.eas.hr.ats.app.team.AttenceResultSum.list'
			    });	
			}else {
				that.pageStep = 0;
				that.changePageLabelColor();
				that.reloadPage({
					uipk: 'com.kingdee.eas.hr.ats.app.team.AttendanceResult.dynamicList'
			    });
			}
		}) ;
...
//			pager : '#gridPager1',
			...
			editurl : this.dynamicPage_url + "?method=editRowData" + "&uipk=com.kingdee.eas.hr.ats.app.team.AttenceResultSum.list"
		...
		options.loadComplete = function(data) {
			...
			shr.ats.team.TeamAtsResultToSumList.superClass.gridLoadComplete.call(self,data);
			setTimeout(function(){
				self.gridId = '#grid';
			},100);
		};
	...
	getGridDataRequestURL : function() {
		return this.dynamicPage_url + "?method=getGridData" + "&uipk=com.kingdee.eas.hr.ats.app.team.AttenceResultSum.list";
	},
...
//		$("#gripage").text($("#gridPager1_left >div[dir='ltr']").text());
	...
	initTabPages: function(){
		...
		$('#calAttendPersonList').click(function(){ 
			...
			//定义标签样式
			that.changePageLabelColor();
			var serviceUrl = "com.kingdee.eas.hr.ats.app.team.AttenceResultSum.list";
			if(currentCalUrl || currentCalUrl != ""){
				serviceUrl = currentCalUrl;
			}else {
				serviceUrl = shr.attenceCalCommon.getAtsCalServiceUrl();
			}
			if(serviceUrl && (serviceUrl == "com.kingdee.eas.hr.ats.app.team.AttendanceResult.dynamicList" || serviceUrl =="com.kingdee.eas.hr.ats.app.team.AttenceResultSum.list")){
				that.reloadPage({
					uipk: serviceUrl
		    	});
			}else {
		...
		$('#sumPersonList').click(function(){ 
			...
			that.changePageLabelColor();
			that.reloadPage({
				uipk: 'com.kingdee.eas.hr.ats.app.team.AttendanceResultSum.dynamicList',
				currentCalUrl: currentCalUrl
		    });	
		});
		...
		$('#salaryPersonList').click(function(){ 
			...
			that.changePageLabelColor();
			that.reloadPage({
				uipk: 'com.kingdee.eas.hr.ats.app.team.AttendanceResultSalary.dynamicList',
				currentCalUrl: currentCalUrl
		    });	
		});
	},
...
//				shr.showWarning({message : "请选择过滤条件执行查询！"});
    	...
    	CalAllAction:function(){  
    		...
    		var url = shr.getContextPath()+"/dynamic.do?handler=com.kingdee.shr.ats.web.handler.dynamic.calculate.AttendanceDynamicCalculateHelper";
    		    url +="&uipk=com.kingdee.eas.hr.ats.app.team.AttendanceResultSum.dynamicList$fragment" ;
    		openLoader(1);
    		 shr.remoteCall({
    			type : "post",
    			url  : url,
...
//			$('input[name=MONTH]').val(curDateM);
		...
		saveDataSalary: function(salaryStatus){
			...
			if(Exchange_json.length>0)
			{
			 ...
			 var url = shr.getContextPath() + "/dynamic.do?uipk=com.kingdee.eas.hr.ats.app.team.AttenceResultSum.list$fragment&serviceId="+serviceId;
			 var methodName=null;
			 if(salaryStatus==1){
