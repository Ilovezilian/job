...
var hideButtonId = [];
shr.defineClass("shr.ats.team.TeamAtsResultCalDynamicList", shr.ats.team.TeamAtsResultDynamicList, {
	isConfirm: false,
	initalizeDOM : function () {
		var _self = this;
		shr.ats.team.TeamAtsResultCalDynamicList.superClass.initalizeDOM.call(_self);
		_self.onNaviLoad();
...
//			shr.showWarning({message : "请选择过滤条件执行查询！"});
	...
	CalAllAction:function(){  
		...
		var url = shr.getContextPath()+"/dynamic.do?handler=com.kingdee.shr.ats.web.handler.dynamic.calculate.AttendanceDynamicCalculateHelper";
		    url +="&uipk=com.kingdee.eas.hr.ats.app.team.AttendanceResult.dynamicList$fragment&serviceId=" +serviceId;
		openLoader(1);
		 shr.remoteCall({
			type : "post",
			url  : url,
	...
	deleteAction: function(){
		...
		var url = shr.getContextPath() + "/dynamic.do?uipk=com.kingdee.eas.hr.ats.app.team.AttendanceResult.dynamicList&method=delete"
		if(ids.length > 0){
