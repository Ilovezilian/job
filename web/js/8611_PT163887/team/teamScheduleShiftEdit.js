shr.defineClass("shr.ats.team.TeamScheduleShiftEdit", shr.framework.Edit, {
	  /*是否触发了真正意义上的改变事件*/
	  isTriggerShiftChange : false,
	  firstDefaultShift : null,
	  isClickShiftF7 : false,
      initalizeDOM:function(){
        shr.ats.team.TeamScheduleShiftEdit.superClass.initalizeDOM.call(this);
			var that = this ;
			//初始化分录数据
			that.initEditGrid();
			...
			if(that.getOperateState() == "EDIT"){
			    ...
			    if($("#matchDaytype_el").val()==0 && !$("#matchShift_el").val()){
						$("#matchDaytype").val("");
						$("#matchDaytype").attr("title","");
					}
			}
			$("#defaultShift").on("focus",function(){$(this).blur()});
			if(that.getOperateState()=="ADDNEW" || that.getOperateState() == "EDIT"){
				that.addElasticDoc();//设置弹性班浮动提示
				that.addElasticHtml();
				if($("#dayType_el").val()==0 && !$("#defaultShift").val()){
					$("#dayType").val("");
				}
				if($("#matchDaytype_el").val()==0 && !$("#matchShift").val()){
					$("#matchDaytype").val("");
				}
			}
			...
			if ($("#dayType").val() == 0 && !$("#defaultShift").val()) {
				$("#dayType").text("");
			}
			if ($("#matchDaytype").val() == 0 && !$("#matchShift").val()) {
				$("#matchDaytype").text("");
			}
			if ($("#isElastic").val() == 0) {
      	 ...
      	 if(that.getOperateState() == "VIEW" || that.getOperateState() == "EDIT"){
			...
			shr.doAjax({
				...
				success: function(response){ 
					...
					that.getOperateState() == "EDIT" && that.doubleTiggerElastic();
      ...
      ,initEditGrid :function(){
	  	var row_fields_work =  '<div style="padding-top:15px;" class="row-fluid row-block row_field">'
			...
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_11
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_3
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_2
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_72
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_76
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_29
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_2
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_83
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_84
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_29
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_88
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_86
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_scheduleShiftEdit_i18n_12
			+ '</span></div>'
	  	 	+ '</div>';
	...
	,addMainData:function(rst){
		...
		if(!atsShift.standardHour){
			...
			$("#elasticDirection").shrSelect("setValue",atsShift.elasticDirection);
			$("#elasticValue").shrTextField("setValue",atsShift.elasticValue);
	  }
	}
	...
	,packageF7Value:function(rowNum,segment_value,attendanceType_value,preIsPunchCard_value,nextIsPunchCard_value,preTimeDayType_value,nextTimeDayType_value){	
		   ...
		   segment.data = [{value:"1",alias:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_10},
		                    {value:"2",alias:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_8},
		            		{value:"3",alias:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_9}];
		   ...
		   attendanceType.data = [{value:"1",alias:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_92},
		                    	  {value:"2",alias:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_30},
		            			  {value:"3",alias:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_93},
		            		      {value:"4",alias:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_31}];
		  ...
		  timeDayType.data = [{value:"0",alias:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_33},
		                    	  {value:"1",alias:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_7},
		            			  {value:"2",alias:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_32}];
		   ...
		   preIsPunchCard.data = [{value:"1",alias:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_78},
		                    	  {value:"0",alias:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_28}];
	...
	,myExtendValidate:function(){ //扩展自定义校验
	      jQuery.extend(jQuery.validator.messages, {  
    		myTmVldt: jsBizMultLan.atsManager_scheduleShiftEdit_i18n_34 ,
    		myRestTmVldt: jsBizMultLan.atsManager_scheduleShiftEdit_i18n_34 ,
    		myRestVldt:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_13,
    		my24Vldt:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_0
		  });  
		  ...
		  }, jsBizMultLan.atsManager_scheduleShiftEdit_i18n_34);//msg:错误提示文本。已验证
		  ...
		  }, jsBizMultLan.atsManager_scheduleShiftEdit_i18n_14);//msg:错误提示文本。已验证
	      ...
	      },jsBizMultLan.atsManager_scheduleShiftEdit_i18n_0);
	}	
	        ...
	        for(var i=0;i<a.length-1;i++){
	           for(var j=i+1;j<a.length;j++){
	              ...
	              if (min1.segment==max1.segment) {
	              	 rt.valid=false;
	                 rt.info=jsBizMultLan.atsManager_scheduleShiftEdit_i18n_1+getSegmentNameByValue(min1.segment);
	                 return rt;
	              }
	           }
	        }   
	        ...
	        for(var i=0;i<a.length-1;i++){
	           for(var j=i+1;j<a.length;j++){
	              ...
	              if( that.validateTimeLegal(min,max))
	              {
	                 ...
	                 rt.info=jsBizMultLan.atsManager_scheduleShiftEdit_i18n_95;
	                 return rt;
	              }
	           }
	        }
	...
	,validateItems:function(){
		...
		if(item.length>1 && $('#isElastic').shrCheckbox("isSelected") && $("input#elasticType").shrSelect("getValue").value==0){
    		shr.showWarning({message:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_37});
    	    return false;
    	}
    	...
    	&& $('#isElastic').shrCheckbox("isSelected") && $("input#elasticType").shrSelect("getValue").value==0){
    		shr.showWarning({message:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_38});
    	    return false;
    	}
    	if($('#isElastic').shrCheckbox("isSelected") && $("input#elasticType").shrSelect("getValue").value==1){
    		if(!(parseFloat(atsMlUtile.getFieldOriginalValue("elasticValue"))>0)){
    			shr.showWarning({message:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_5});
    	        return false;
    		}
    		if(!(/^-?\d+\.?\d{0,2}$/.test(atsMlUtile.getFieldOriginalValue("elasticValue")))){
    			shr.showWarning({message:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_6});
    	        return false;
    		}
    		if(parseFloat(atsMlUtile.getFieldOriginalValue("elasticValue"))>=parseFloat(atsMlUtile.getFieldOriginalValue("standardHour"))){
    			shr.showWarning({message:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_81});
    	        return false;
    		}
    		...
    		if ( !(restpretime.trim() == "")){
				...
				if(restpretimeVal>pretimeVal && parseFloat(atsMlUtile.getFieldOriginalValue("elasticValue"))>=(restpretimeVal-pretimeVal) ){
					shr.showWarning({message:jsBizMultLan.atsManager_scheduleShiftEdit_i18n_94});
    	    		return false;
				}
	...
	,saveAction: function(event) {
		...
		var _self = this;
		$("#elasticDirection").val($("#elasticDirection").shrSelect("getValue").value);
		atsMlUtile.setTransNumValue("elasticValue",atsMlUtile.getFieldOriginalValue("elasticValue"));
		if(dayType == '0' ){
			...
			var dayType_el = $('#dayType_el').val();
			var daytype = $('#dayType').val();
			if(dayType_el == null || dayType_el == undefined || dayType_el.length < 1 ||
				(dayType_el == "0" && (daytype == null || daytype == undefined || "" == daytype))){
				shr.showWarning({message: jsBizMultLan.atsManager_scheduleShiftEdit_26522380_i18n_0});
				return false;
			}
			var defaultShift_el = $('#defaultShift_el').val();
			if(defaultShift_el == null || defaultShift_el == undefined || defaultShift_el.length < 1){
				shr.showWarning({message: jsBizMultLan.atsManager_scheduleShiftEdit_i18n_35});
				return false;
			}
		}
		else{
	...
	calSegmentInRestValue : function(restPreTime,restNextTime,preTime,nextTime,predaytype,nextdaytype) {
		...
		if (srartdate.getTime()<predate.getTime() && senddate.getTime()<predate.getTime()) {
			rt.valid = false;
			rt.info = jsBizMultLan.atsManager_scheduleShiftEdit_i18n_89;
		}
		
		if (srartdate.getTime()<predate.getTime() || srartdate.getTime()>nextdate.getTime()) {
			rt.valid = false;
			rt.info = jsBizMultLan.atsManager_scheduleShiftEdit_i18n_90;
		}
		if (senddate.getTime()<predate.getTime() || senddate.getTime()>nextdate.getTime()) {
			rt.info = jsBizMultLan.atsManager_scheduleShiftEdit_i18n_87;
			rt.valid = false;
		}
	...
	valideTimeOfCompare: function(vo){
			...
			if (parseInt(prevalue) == 0 && parseInt(nextvalue) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_74});
				return false
			}
			if (parseInt(prevalue) == 0 && parseInt(nextvalue) == 2) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_75});
				return false;
			}
			if (parseInt(prevalue) == 1 && parseInt(nextvalue) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_71});
				return false;
			}
			if (parseInt(prevalue) == 2) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_73});
				return false;
			}
		}
		//二段班
		if (vo.length == 2) {
			...
			if (offtime1.getTime() < ontime1.getTime()) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_24});
				return false;
			}
			if (offtime2.getTime() < ontime2.getTime()) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_18});
				return false;
			}
			...
			if (parseInt(prevalue1) == 0 && parseInt(nextvalue1) == 2) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_27});
				return false;
			}
			...
			if (parseInt(prevalue1) == 0 && (parseInt(nextvalue2) == 0 || parseInt(nextvalue2) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_23});
				return false;
			}
			//中间组合情况判断
			if (parseInt(nextvalue1) == 1 && parseInt(prevalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_25});
				return false;
			}
			if (parseInt(nextvalue1) == 1 && parseInt(nextvalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_26});
				return false;
			}
			
			//上班时间为当天
			if (parseInt(prevalue1) == 1 && parseInt(nextvalue1) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_22});
				return false;
			}
			if (parseInt(prevalue1) == 1 && parseInt(prevalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_20});
				return false;
			}
			if (parseInt(prevalue1) == 1 && parseInt(nextvalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_21});
				return false;
			}
			//中间组合情况判断
			if (parseInt(nextvalue1) == 1 && parseInt(prevalue2) == 0) {
			...
			if (parseInt(prevalue2) == 2 && (parseInt(nextvalue2) == 0 || parseInt(nextvalue2) == 1) ) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_17});
				return false;
			}
		...
		if (vo.length == 3) {
			...
			if (offtime1.getTime() < ontime1.getTime()) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_66});
				return false;
			}
			if (offtime2.getTime() < ontime2.getTime()) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_42});
				return false;
			}
			if (offtime3.getTime() < ontime3.getTime()) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_50});
				return false;
			}
			
			if (parseInt(prevalue1) == 0 && parseInt(nextvalue1) == 2) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_65});
				return false;
			}
			if (parseInt(prevalue1) == 0 && (parseInt(prevalue2) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_57});
				return false;
			}
			if (parseInt(prevalue1) == 0 && parseInt(nextvalue1) == 1 && (parseInt(prevalue2) == 0 || parseInt(prevalue2) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_58});
				return false;
			}
			//---
			if (parseInt(prevalue1) == 0 && (parseInt(nextvalue2) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_59});
				return false;
			}
			if (parseInt(prevalue1) == 0 && parseInt(nextvalue1) == 1 && (parseInt(nextvalue2) == 0 || parseInt(nextvalue2) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_60});
				return false;
			}
			...
			if (parseInt(prevalue1) == 0 && (parseInt(prevalue3) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_61});
				return false;
			}
			if (parseInt(prevalue1) == 0 && parseInt(nextvalue1) == 1 && (parseInt(prevalue3) == 0 || parseInt(prevalue3) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_62});
				return false;
			}
			...
			if (parseInt(prevalue1) == 0 && (parseInt(nextvalue3) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_63});
				return false;
			}
			if (parseInt(prevalue1) == 0 && parseInt(nextvalue1) == 1 && (parseInt(nextvalue3) == 0 || parseInt(nextvalue3) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_64});
				return false;
			}
			...
			//上班时间为当天
			if (parseInt(prevalue1) == 1 && parseInt(nextvalue1) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_56});
				return false;
			}
			if (parseInt(prevalue1) == 1 && parseInt(prevalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_52});
				return false;
			}
			if (parseInt(prevalue1) == 1 && parseInt(nextvalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_53});
				return false;
			}
			if (parseInt(prevalue1) == 1 && parseInt(prevalue3) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_54});
				return false;
			}
			if (parseInt(prevalue1) == 1 && parseInt(nextvalue3) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_55});
				return false;
			}
			//中间组合情况判断
			if (parseInt(nextvalue1) == 1 && parseInt(prevalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_67});
				return false;
			}
			if (parseInt(nextvalue1) == 1 && parseInt(nextvalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_68});
				return false;
			}
			if (parseInt(nextvalue1) == 1 && parseInt(prevalue3) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_69});
				return false;
			}
			if (parseInt(nextvalue1) == 1 && parseInt(nextvalue3) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_70});
				return false;
			}
			
			if (parseInt(prevalue2) == 2 && (parseInt(nextvalue2) == 0 || parseInt(nextvalue2) == 1) ) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_41});
				return false;
			}
			
			if (parseInt(nextvalue2) == 1 && parseInt(prevalue3) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_43});
				return false;
			}
			if (parseInt(nextvalue2) == 1 && parseInt(nextvalue3) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_44});
				return false;
			}
			
			if (parseInt(nextvalue2) == 2 && (parseInt(prevalue3) == 0 || parseInt(prevalue3) ==1) ) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_45});
				return false;
			}
			if (parseInt(nextvalue2) == 2 && (parseInt(nextvalue3) == 0 || parseInt(nextvalue3) == 1) ) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_46});
				return false;
			}
			
			if (parseInt(prevalue3) == 2 && (parseInt(nextvalue3) == 0 || parseInt(nextvalue3) == 1) ) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_49});
				return false;
			}
	...
	valideTimeOfCompareNew: function(vo){
			...
			if (newVo.length == 1) {
				...
				if(temp.preDateTime.getTime()>temp.nextDateTime.getTime()){
				  shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_91});
				  return false;
				}
				//加上不能大于24小时的判断
				if(temp.nextDateTime.getTime()-temp.preDateTime.getTime()>1*24*60*60*1000){
				  shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_95});
				  return false;
				}
			}
			...
			if (newVo.length == 2) {
				...
				if(preDateTime1.getTime()>nextDateTime1.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_19});
				   return false;
				}
				
				if(preDateTime2.getTime()<nextDateTime1.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_16});
				   return false;
				}
				
				if(preDateTime2.getTime()>nextDateTime2.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_15});
				   return false;
				}
				//加上不能大于24小时的判断
		  ...
		  if (newVo.length == 3) {
			   ...
			   if(preDateTime1.getTime()>nextDateTime1.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_51});
				   return false;
				}
				
				if(preDateTime2.getTime()<nextDateTime1.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_40});
				   return false;
				}
				
				if(preDateTime2.getTime()>nextDateTime2.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_39});
				   return false;
				}
				
				if(preDateTime3.getTime()<nextDateTime2.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_48});
				   return false;
				}
				
				if(preDateTime3.getTime()>nextDateTime3.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_scheduleShiftEdit_i18n_47});
				   return false;
				}
	     ...
	     $('#dayType').on("change", function(){
				...
				if(dayType == '0' ){
					...
					shr.doAjax({
						...
						success: function(res){ 
							atsMlUtile.setTransNumValue('standardHour',res.standardHour);
						}
					});
	...
	,addElasticDoc: function() {
		$("#elasticType_down").find("a").eq(0).attr("title",jsBizMultLan.atsManager_scheduleShiftEdit_i18n_36);
		$("#elasticType_down").find("a").eq(1).attr("title",jsBizMultLan.atsManager_scheduleShiftEdit_i18n_77);
	}
	,addElasticHtml:function(){
	    ...
	    if($("#elasticType").shrSelect("getValue").value==0){
	    	$('#elasticValue').closest('div[data-ctrlrole="labelContainer"]').hide();
	    	$('#elasticDirection').closest('div[data-ctrlrole="labelContainer"]').hide();
	    }
		...
		$("#elasticType").change(function(){
			...
			$("input[name^='preFloatAdjusted']").attr("disabled","disabled");
			$("input[name^='nextFloatAdjusted']").attr("disabled","disabled");
			if($(this).shrSelect("getValue").value==0){
				$('#elasticValue').closest('div[data-ctrlrole="labelContainer"]').hide();
				$('#elasticDirection').closest('div[data-ctrlrole="labelContainer"]').hide();

				$("input[name^='restPreTime']").attr("disabled","disabled");
				$("input[name^='restNextTime']").attr("disabled","disabled");

			}else{
				$('#elasticValue').closest('div[data-ctrlrole="labelContainer"]').show();
				$('#elasticDirection').closest('div[data-ctrlrole="labelContainer"]').show();
				if($("#itemInfo").find(".row-fluid").length<=2){
					$("input[name^='restPreTime']").removeAttr("disabled");
					$("input[name^='restNextTime']").removeAttr("disabled");
				}
			}
		});

		$("input#isElastic").shrCheckbox('onChange', function(){
        	if($('#isElastic').shrCheckbox("isSelected")){
			   ...
			   $("#elasticType").change();
				...
				$("input[name^='preFloatAdjusted']").removeAttr("disabled");
				$("input[name^='nextFloatAdjusted']").removeAttr("disabled");
        	    if($("#itemInfo").find(".row-fluid").length<=2){
	        	   $("input[name^='restPreTime']").removeAttr("disabled");
	        	   $("input[name^='restNextTime']").removeAttr("disabled");
        	    }
        	...
        	$('#isElastic').shrCheckbox('disable');
        	$('#elasticDirection').shrSelect("disable");
        	$('#elasticValue').shrTextField("disable");
        });
...
function getSegmentNameByValue(val){
   if("1" == val){
     return jsBizMultLan.atsManager_scheduleShiftEdit_i18n_10;
   }else if("2" == val){
     return jsBizMultLan.atsManager_scheduleShiftEdit_i18n_8;
   }else if("3" == jsBizMultLan.atsManager_scheduleShiftEdit_i18n_9){
     return jsBizMultLan.atsManager_scheduleShiftEdit_i18n_9;
   }
}
...
function getDayTypeValueByName(typeName){
   if(jsBizMultLan.atsManager_scheduleShiftEdit_i18n_33 == typeName){
        return "0";
   }else if(jsBizMultLan.atsManager_scheduleShiftEdit_i18n_7 == typeName){
   	    return "1";
   }else if(jsBizMultLan.atsManager_scheduleShiftEdit_i18n_32 == typeName){
   	    return "2"
   }else{
        return "";
   }
