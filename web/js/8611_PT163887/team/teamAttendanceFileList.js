shr.defineClass("shr.ats.team.TeamAttendanceFileList", shr.ats.lackAttendanceFileInfoEdit, {
    pageStep: 0,
	initalizeDOM : function () {
        shr.ats.team.TeamAttendanceFileList.superClass.initalizeDOM.call(this);
		var that = this;
		that.divDialog();
		that.initTabPages();
	...
	viewAction: function(billId) {
		this.reloadPage({
            uipk: 'com.kingdee.eas.hr.ats.app.team.AttendanceFileView.form',
			billId: billId,
			method: 'view' 
		});		
	}, 
	...
	enableFileAction: function() {
		...
		if (selectedIds.length > 0) {
			...
			for (var i = 0, length = selectedIds.length; i < length; i++) {
				...
				if(state==1){
					shr.showInfo({message: jsBizMultLan.atsManager_attendanceFileList_i18n_9, hideAfter: 3});
					return;
				}
				if(bills){
	...
	deleteAction:function(){	
		...
		if (selectedIds.length > 0) {
            ...
            shr.ats.team.TeamAttendanceFileList.superClass.deleteAction.call(this);
		}else{
			shr.showInfo({message: jsBizMultLan.atsManager_attendanceFileList_i18n_30, hideAfter: 3});
			return;
		}
...
//			  uipk: 'com.kingdee.eas.hr.ats.app.AttendanceFileDisPersonViewV2.list'
	...
	addValsAction : function(){
        ...
        var pageUipk = "com.kingdee.eas.hr.ats.app.team.AttendanceFile.list";//考勤档案列表
        var _self = this;
