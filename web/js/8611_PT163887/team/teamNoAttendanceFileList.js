shr.defineClass("shr.ats.team.TeamNoAttendanceFileList", shr.framework.List,{
	pageStep: 1,
    initalizeDOM : function() {
        shr.ats.team.TeamNoAttendanceFileList.superClass.initalizeDOM
				.call(this);
		var that = this;
		that.initTabPages();
		that.setNavigate();
		$("#addNew").attr("title", jsBizMultLan.atsManager_noAttendanceFileList_i18n_2);
		$("#addAll").attr("title", jsBizMultLan.atsManager_noAttendanceFileList_i18n_2);
	...
	addNewAction : function(event) {
		...
		if (billIds == undefined || billIds.length == 0) {
			shr.showWarning({message : jsBizMultLan.atsManager_noAttendanceFileList_i18n_1});
			return;
		}
		...
		that.remoteCall({
						...
						//	if(res.flag){
                that.reloadPage({uipk : 'com.kingdee.eas.hr.ats.app.team.AttendanceFileDisPersonSaveAllV2.list'});
						//	}
						//	else{
	...
	cancelAllAction : function() {
        this.reloadPage({uipk : 'com.kingdee.eas.hr.ats.app.team.AttendanceFile.list'});
	},
	...
	initTabPages: function(){
	    ...
	    $('#attendanceFileList').click(function(){ 
			...
			that.changePageLabelColor();
			that.reloadPage({
                uipk: 'com.kingdee.eas.hr.ats.app.team.AttendanceFile.list'
		    });	
		});
...
//			  uipk: 'com.kingdee.eas.hr.ats.app.AttendanceFileDisPersonViewV2.list'
                uipk: 'com.kingdee.eas.hr.ats.app.team.NoAttendanceFile.list'
		    });	
		});
	...
	setNavigate:function(){
		...
		$("#breadcrumb li").eq($("#breadcrumb li").length-1).html(jsBizMultLan.atsManager_noAttendanceFileList_i18n_0);
	}
	,getPersonChangeTabNum : function(){
		 var _self = this;
		 var serviceId = shr.getUrlRequestParam("serviceId");
        var uipk = "com.kingdee.eas.hr.ats.app.team.NoAttendanceFile.list";
		 var url = shr.getContextPath() + "/dynamic.do?uipk="+uipk+"&method=getPersonChangeTabNum";
		 url += '&serviceId='+encodeURIComponent(serviceId);
		  var _longNumber = $("#treeNavigation").shrGridNavigation('getValue').longNumber;
		 shr.ajax({
				type:"post",
				async:true,
