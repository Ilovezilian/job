shr.defineClass("shr.ats.team.TeamAttendanceFileDisPersonSaveAllV2List",shr.ats.team.TeamAttendanceFileBatchMaintainBase, {
	initalizeDOM : function () {
		shr.ats.team.TeamAttendanceFileDisPersonSaveAllV2List.superClass.initalizeDOM.call(this);
		var that = this;
		//隐藏精确搜索框
	...
	saveAllAction:function(){
		...
		for(var i=0;i<gridData.length;i++){
			if(gridData[i]['hrOrgUnit.id']==null || gridData[i]['hrOrgUnit.id']==''){
				shr.showInfo({message: jsBizMultLan.atsManager_attendanceFileDisPersonSaveAllV2List_i18n_4});
				return false;
			}
			if(gridData[i]['attPosition.id']==null || gridData[i]['attPosition.id']==''){
				shr.showInfo({message: jsBizMultLan.atsManager_attendanceFileDisPersonSaveAllV2List_i18n_5});
				return false;
			}
			if(gridData[i]['attendanceNum']==null || gridData[i]['attendanceNum']==''){
				shr.showInfo({message: jsBizMultLan.atsManager_attendanceFileDisPersonSaveAllV2List_i18n_1});
				return false;
			}
			if(gridData[i]['atsShift.id']==null || gridData[i]['atsShift.id']==''){
				shr.showInfo({message: jsBizMultLan.atsManager_attendanceFileDisPersonSaveAllV2List_i18n_7});
				return false;
			}
			if(gridData[i]['attencePolicy.id']==null || gridData[i]['attencePolicy.id']==''){
				shr.showInfo({message: jsBizMultLan.atsManager_attendanceFileDisPersonSaveAllV2List_i18n_6});
				return false;
			}
			if(gridData[i]['isAttendance']==null || gridData[i]['isAttendance']===''){
				shr.showInfo({message: jsBizMultLan.atsManager_attendanceFileDisPersonSaveAllV2List_i18n_0});
				return false;
			}
			if(gridData[i]['isAutoShift']==null || gridData[i]['isAutoShift']===''){
				shr.showInfo({message: jsBizMultLan.atsManager_attendanceFileDisPersonSaveAllV2List_i18n_9});
				return false;
			}
			if(gridData[i]['EFFDT']==null || gridData[i]['EFFDT']==''){
				shr.showInfo({message: jsBizMultLan.atsManager_attendanceFileDisPersonSaveAllV2List_i18n_8});
				return false;
			}
		}
		...
		openLoader(1,jsBizMultLan.atsManager_attendanceFileDisPersonSaveAllV2List_i18n_10);
		shr.ajax({
			type:"post",
			async:true,
			url:url,
...
/*	saveAllAction: function() {
	...
	setNavigate:function(){
	   ...
	   for(var i=0;i<$("#breadcrumb li").length-1;i++){
	       if(i!=0){
		      if($("#breadcrumb li a").eq(i).html() == jsBizMultLan.atsManager_attendanceFileDisPersonSaveAllV2List_i18n_3){
			     $("#breadcrumb li a").eq(i).html(jsBizMultLan.atsManager_attendanceFileDisPersonSaveAllV2List_i18n_2);
				 disFileIndex=i;
			  }
