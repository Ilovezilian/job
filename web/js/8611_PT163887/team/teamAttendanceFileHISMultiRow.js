...
$().ready(function() {
	...
	$("#warmTip").html($.attendmanageI18n.attendanceFileHISMultiRow.msg1)
});
shr.defineClass("shr.ats.team.TeamAttendanceFileHIS", shr.framework.MultiRow, {
	
	initalizeDOM:function(){
	
	   shr.ats.team.TeamAttendanceFileHIS.superClass.initalizeDOM.call(this);
	   var that = this ;
...
//	   }
	   if(that.getOperateState() == 'EDIT'){
		   uuid = that.uuid; 
		   $("#attencePolicy"+uuid).shrPromptBox("setBizFilterFields","hrOrgUnit"+uuid);
		   $("#attencePolicy"+uuid).shrPromptBox("setBizFilterFieldsValues",$("#hrOrgUnit"+uuid).shrPromptBox("getValue").id);
		   
		   $("#atsShift"+uuid).shrPromptBox("setBizFilterFields","hrOrgUnit"+uuid);
		   $("#atsShift"+uuid).shrPromptBox("setBizFilterFieldsValues",$("#hrOrgUnit"+uuid).shrPromptBox("getValue").id);

		   $("#attPosition"+uuid).shrPromptBox("setBizFilterFields","hrOrgUnit"+uuid);
		   $("#attPosition"+uuid).shrPromptBox("setBizFilterFieldsValues",$("#hrOrgUnit"+uuid).shrPromptBox("getValue").id);
		   ...
		   $("#position"+uuid).shrPromptBox("option", {
			...
			//考勤职位
			$("#attPosition"+uuid).shrPromptBox("option",{
				onchange : function(e, value) {
					var info = value.current; 
					var url = shr.getContextPath() + "/dynamic.do?handler=com.kingdee.shr.ats.web.handler.AttendanceFileEditHandler&method=getOrgUnitByPosition";
					 shr.ajax({
							...
							{
							 	var info = res;
								$("#attAdminOrgUnit"+uuid+"_el").val( info.adminOrgUnitId);
								$("#attAdminOrgUnit"+uuid).val(info.adminOrgUnitName);
							}
				 	});
				}
			});
...
//	}
	,closeAllMainAction:function(){
		this.reloadPage({
			uipk: 'com.kingdee.eas.hr.ats.app.team.AttencePolicy.AvailableList.F7'
		});
	}
	/**
	 * 保存
	...
	,saveAllMainAction: function() {
		...
		for(var i=0;i<entryLen;i++){
			...
			if($("#attendanceNum"+uuid).val()==""||$("#attendanceNum"+uuid).val()==undefined||
			   ...
			   $("#LEFFDT"+uuid).val()==""||$("#LEFFDT"+uuid).val()==undefined
				...
				shr.showInfo({message: $.attendmanageI18n.attendanceFileHISMultiRow.msg2, hideAfter: 3});
				return;
			}
		}
		...
		if(boo){
			shr.showInfo({message: $.attendmanageI18n.attendanceFileHISMultiRow.msg3, hideAfter: 3});
			return;
		}
		for(var i=0;i<entryLen;i++){
...
//			if(count==entryLen){
//				 shr.ats.team.TeamAttendanceFileHIS.superClass.reloadPage({
//					uipk: "com.kingdee.eas.hr.ats.app.team.TeamAttendanceFileHIS.form",
...
//				shr.showInfo({message: "生效日期不能大于当前日期!", hideAfter: 5});
//				return false;
//			}
			var leffdt =  $("#LEFFDT"+uuid).val();
			if(nextdate){
				if(new Date(effdt.replace(/-/g,"/")).valueOf()>=nextdate.valueOf()){
					shr.showInfo({message: $.attendmanageI18n.attendanceFileHISMultiRow.msg5, hideAfter: 5});
					return false;
				}
			}
			if(predate){
		...
		if(boo){
			shr.showInfo({message: $.attendmanageI18n.attendanceFileHISMultiRow.msg6, hideAfter: 5});
			return false;
		}else{
			return true;
		}
