...
var _rowMap ;
shr.defineClass("shr.ats.team.TeamAtsLeaveBillBatchEdit", shr.ats.atttenceEditFormImport, {
	rowid: "",
	cellname: "",
	value: "",
	iRow: "",
	...
	initalizeDOM: function () {
		shr.ats.team.TeamAtsLeaveBillBatchEdit.superClass.initalizeDOM.call(this);
		var that = this;
		...
		$('<button type="button" style="margin-left:4px;margin-right:4px" class="shrshrbtn-primary shrbtn" id="orgfill">' 
				+ jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_11 
		...
		$('<button type="button" style="margin-left:4px;margin-right:4px;display: none" class="shrshrbtn-primary shrbtn" id="import">' 
				+ jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_1 
				+ '</button>').insertAfter($("#addRow_entries"));
		...
		$('.editGrid-toolbar').append('<div style="display: inline;padding-left: 20px;">' 
				+ jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_33 
		...
		$('#orgfill').click(function () {
			...
			$('#hasNum').val(gridNum);
			$('#orgFillDiv').dialog({
				title: jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_11,
				width: 1020,
				height: 450,
				modal: true,
				resizable: false,
...
//				}
				,buttons:[{
						text: jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_23,
						click: function() {
					    if (that.checkF7Data()) {
							openLoader(1);
							// 给jqgrid 赋值
						...
						text: jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_3,
						click: function() {
							$(this).dialog( "close" );
						}
					}]
		...
		if (that.getOperateState() == 'VIEW') {
			if (that.isFromWF()) { // 来自任务中心
				$('#submitEffect').hide();
				$('#cancelAll').hide();
				$("#submit").text(jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_27);
			}
			var rowIds = $("#entries").getDataIDs();
			var that = this;
			$.each(rowIds, function (n, value) {
...
//					that.emptyHrOrgBasedEntryData();
	...
	,createNewEntryModel: function() {
		var lastRowNum = $('#entries').getGridParam("reccount");
		if (lastRowNum>=100) {
			shr.showWarning({message:jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_29,hideAfter:5});
			return false;
		} else {
...
//				var lastRemark = $("#entries").jqGrid("getRowData", temp_id).remark;
				return {beginTime:beginTime,endTime:endTime,reason:leaveReason};
			}		
		}		
	}
	...
	, submitAction: function (event) {
		...
		if ($form.valid() && _self.verify()) {
			shr.showConfirm(jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_9, function () {
	...
	, submitEffectAction: function (event) {
		...
		if ($form.valid() && _self.verify()) {
			if (shr.atsBillUtil.isInWorkFlow(_self.billId)) {
				shr.showConfirm(jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_2, function () {
					_self.prepareSubmitEffect(event, 'submitEffect');
				});
			} else {
				shr.showConfirm(jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_10, function () {
					_self.prepareSubmitEffect(event, 'submitEffect');
				});
			}
		}
	...
	, cancelAllAction: function () {
		/*var that = this ;
	 	window.location.href = shr.getContextPath()+"/dynamic.do?uipk=com.kingdee.eas.hr.ats.app.team.AtsLeaveBillAll.list";*/
		this.reloadPage({
...
//		if (that.checkExistSamePeron()) {
//			return false;
//		}
//		if(!that.checkEnoughRemain()){
//			shr.showWarning({message: "单人请假总时长大于剩余额度！"});
//			return false;
//		}
		_rowMap = {} ;
		var errorString = "";
	...
	, checkF7Data: function () {
		...
		*/
		...
		if (parseInt(checkAmount) > parseInt(remain)) {
			shr.showWarning({ message: jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_34 });
			return false;
		}
		//校验
		if (holidayType_el == undefined || holidayType_el == null || holidayType_el == ""
			|| holidayType == undefined || holidayType == null || holidayType == "") {
			shr.showWarning({ message: jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_5 });
			return false;
		}
		...
		if ((adminOrgUnit_el == undefined || adminOrgUnit_el == null || adminOrgUnit_el == ""
			...
			shr.showWarning({ message: jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_32 });
			return false;
		}

		if (beginTime == undefined || beginTime == null || beginTime == "") {
			shr.showWarning({ message: jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_8 });
			return false;
		}

		if (leaveLength == undefined || leaveLength == null || leaveLength == "") {
			shr.showWarning({ message: jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_13 });
			return false;
		}

		if (endTime == undefined || endTime == null || endTime == "") {
			shr.showWarning({ message: jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_6 });
			return false;
		}
	...
	, setButtonVisible: function () {
		...
		if (billState) {
			if (billState == 3 || 
					jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_25 == billState || 
					billState == 4 || 
					jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_24 == billState || 
					billState == 2 ||
					jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_26 == billState) {
				$("#edit").hide();
				$("#submit").hide();
				$("#submitEffect").hide();
			} else if (1 == billState || 
					jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_28 == billState || 
					2 == billState || 
					jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_26 == billState) { //未审批或审批中
				if (!this.isFromWF()) {
					$("#edit").hide();
					$("#submit").hide();
					$("#submitEffect").hide();
		...
		that.remoteCall({
			...
			success: function (res) {
				...
				$("#entries").find("td[aria-describedby='entries_prompt']").each(function () {
					...
					var showString = shr.formatMsg(jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_0, [info.leaveTimes, info.lastLeaveDate.substr(0, 10)]);
					if (rowid == rowID) {
						$(this).html("");
	...
	, checkIsNullEveryRow: function (value) { // 非空的是框架处理的，这里只要处理 卡死hi时间和结束时间的校验
		...
		if (beginDate.getTime() >= endDate.getTime()) {
			return errorString = jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_7;
		}
		return "";
	}
	//请假时长不能大于计算时长
	, checkCalLength: function (value) {
		if (parseFloat(value.leaveLength) > parseFloat(value.caleaveLength)) {
			return errorString = jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_19 + value.caleaveLength;
		}
		return "";
	}
	, checkIsChangeCalLength: function (rowid) {
		...
		if (realLen > calLeave) { // 请假时长不能大于计算时长 ！
			that.preShowError(rowid, jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_20);
		} else if (realLen < calLeave) { // ？ 
			that.preShowWarn(rowid, jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_21);
		}
	}
...
//		errorString = that.validateIsExistBill(value.person.id, value.beginTime, value.endTime, $('#id').val(), value["policy.holidayType"].id);//f7改变,筛选数据字段发生改变
	...
	var vn=(new String(v)).substring(10,11);
	if(vn ==" "){
	...
	, validateIsExistBill: function (personId, beginTime, endTime, billId, holidayTypeId) {
		...
		var errorString = "";
		var beginFlag=that.checkTime(beginTime);
		var endFlag=that.checkTime(endTime);
		if(!beginFlag){
		 return jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_18;
		}
		if(!endFlag){
		return jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_17;
		}
		that.remoteCall({
			type: "post",
			async: false,
		...
		if (that.getOperateState() == 'ADDNEW') {
			...
			if (infoLeaveBillNumber.leaveBillExist == "true") {
				errorString = shr.formatMsg(jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_30, [infoLeaveBillNumber.leaveBillNumber]);
				return errorString;
			}
		}
		if(info.errorMsg != ""){
		errorString =info.errorMsg;
		return errorString;
		}
		if (info.addFlag > 0) {
			errorString = shr.formatMsg(jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_31, [info.billNo, info.personName, info.realBeginDate, info.realendDate]);
			return errorString;
		}
	}
	//验证请假周期
	, validateLeaveBillCycle: function (personId, beginTime, endTime, holidayTypeId, leaveLength) {
		...
		that.remoteCall({
			...
			param: { billId: $('#id').val(), personId: personId, beginTime: beginTime, endTime: endTime, holidayTypeId: holidayTypeId, leaveLength: leaveLength, hrOrgUnitId: hrOrgUnitId },
			success: function (res) {
				info = res;
				return info;
			}
...
, removePreShowError: function (rowid) {
	...
	$("#entries tr[id='" + rowid + "']").css("color", "rgb(153, 153, 153)");//如果设置成css("color","initial")会变成黑色，而原界面为灰色
...
},checkEveryRowee : function(rownum,value){
			var that = this ;
			var regEx = new RegExp("\\-","gi"); //i不区分大小写 g匹配所有
			
		 	var startTime = value.beginTime ;
			var endTime = value.endTime ;
			
			var startTimeOfDate = NewLongDate(startTime); 
		 	var endTimeOfDate = NewLongDate(endTime);
		 	
		 	var longTime = endTimeOfDate.getTime() - startTimeOfDate.getTime();
		 	
		 	var overTimeBegin = value.beginTime ;
		 	...
		 	for(var prop in _rowMap){
 			   if(_rowMap.hasOwnProperty(prop)){
 			   		...
 			   		if(prop!=rownum&&_rowMap[prop][0]==value.person.id&&!(beginTimeTemp.getTime()>=endTimeOfDate.getTime()||endTimeTemp.getTime()<=startTimeOfDate.getTime())){
 			   			return shr.formatMsg(jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_50, [prop]);
 			   		
 			   		}
    			}
			}
			},showErrors:function(iRow,errorString){
			...
			if (errorString){
				...
				$("#entries tr:eq("+iRow+")").css("color","red");	
			}
	},
	...
	, checkExistSamePeron: function () {
		var personCollTemp = new Array();
		var errorFlag = 0;
		var obj = $("#entries").jqGrid("getRowData");
		for(var i=0;i<obj.length;i++){
			var personNum1 = obj[i].person.id;
			var beginTime1 = obj[i].beginTime;
			var endTime1 = obj[i].endTime;		
			
			for(var k=0;k<obj.length;k++){
				var personNum2 = obj[k].person.id;
				if( i!=k && personNum1 == personNum2){
					var beginTime2 = obj[k].beginTime;
					var endTime2 = obj[k].endTime;						
					var b1 = new Date(beginTime1);
					var e1 = new Date(endTime1);
					var b2 = new Date(beginTime2);
					var e2 = new Date(endTime2);

					if(!(e1<=b2||b1>=e2)){						
						//var personName = $("tr[id='" + billIdArry[k] + "'] td[aria-describedby='grid_person.name']").text();						
						//shr.showWarning({"message" : "[" + personName + ']的请假单时间有交叉！'});
						shr.showWarning({"message" : jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_50});
						return true;
					}										
				}
			}
	...
	, checkRowIsOver: function () {
		if ($("#entries").getGridParam("reccount") > 100) {
			shr.showWarning({ message: jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_14 });
			return false;
		} else {
			return true;
		}
	...
	, showMoreMessage: function (rowid) {
		...
		dialog_Html = "<div id='dialogViewMore' style = 'font-size: 12px; padding: 10px; width: 93%' title=''>" +
			...
			+ jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_12 
			+ "</a></span>" + //onClick='pre()' 
			"		<span style='float:right;display:block;'><a id='a_next' style='cursor:pointer'>" 
			+ jsBizMultLan.atsManager_atsLeaveBillBatchEdit_i18n_4 
			+ "</a></span>" + //onClick='next()'
			"	</h2>" +
	...
	, getCurrentModel: function () {
		...
		var that = this;
		var model = shr.ats.team.TeamAtsLeaveBillBatchEdit.superClass.getCurrentModel.call(this);
		...
		for (var i = 0; i < len; i++) {
			...
			model.entries[i].isCancelLeave= 0;
	...
	,assembleSaveData: function(action) {
		var _self = this;
		var data = shr.ats.team.TeamAtsLeaveBillBatchEdit.superClass.assembleSaveData.call(this,action);
