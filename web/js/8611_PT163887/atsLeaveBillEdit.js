$().ready(function () {
    ...
    $.validator.messages.maxlength = $.validator.format(
        jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_63
    );
...
var beginTime = "";
var beginTimeTimePart = "";
var endTime = "";
var endTimeTimePart = "";

var customizeBF = "8r0AAAA09p9IT3K3";
var bfHolidayTypeId = "8r0AAAAMhrv9LVKY";
var attendTypeIdId = "0";
function adminOrgBind() {
    if ($("#entries_adminOrgUnit").data("data.f7")) {
        var adminOrg = $("#entries_adminOrgUnit").data("data.f7").id;
        var entries_person = $("#entries_person").data("data.f7").id;
        $("#ccPersonIds").shrPromptBox("setOtherParams", {
            adminOrgUnit: adminOrg,
            entries_person: entries_person
        });
    }
}
...
shr.defineClass("shr.ats.AtsLeaveBillEdit", shr.framework.Edit, {
    ...
    initalizeDOM: function () {
        ...
        if (that.getOperateState() == "VIEW") {
            ...
            if (atsMlUtile.getFieldOriginalValue("entries_beginTime") != "") {
                ...
                atsMlUtile.setTransDateTimeValue(
                    "entries_beginTime",
        ...
        if (that.getOperateState() == "VIEW") {
            ...
            $("#extraBfType .row-fluid.row-block")
                .eq(0)
                .css("padding-left", "0");
        ...
        if (that.getOperateState() != "VIEW") {
            ...
            //业务组织onchange事件
            if (that.uipk != "com.kingdee.eas.hr.ats.app.AtsLeaveBillForm") {
        ...
        if (that.getOperateState() == "VIEW") {
            if (that.isFromWF()) {
                ...
                $("#cancelAll").hide();
                $("#submit").text(
                    jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_51
                );
                $("#edit").hide();
            }
        }
        ...
        if (
            ...
            if (shrDataManager.pageNavigationStore.getDatas().length == 4) {
                $($("#breadcrumb li")[3]).html(
                    jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_9
                );
            }
        }
        ...
        if (paramMethod == null) {
            $("#breadcrumb")
                .find(".active")
                .text(jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_55);
    ...
    initStartAndEnd: function () {
        ...
        var personId =
            $("#entries_person_el").val() || $("#entries_person").val();
        ...
        _self.remoteCall({
            ...
            success: function (res) {
                ...
                if (!info.isHaveFile) {
                    shr.showWarning({
                        message:
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_130
                    });
                }
            }
        });
    ...
    getEndTime: function () {
        var that = this;
        var leavelength = atsMlUtile.getFieldOriginalValue(
            "entries_leaveLength"
        );
        ...
        that.remoteCall({
            ...
            success: function (res) {
                ...
                atsMlUtile.setTransDateTimeValue(
                    "entries_endTime",
                    res.atsLeaveEndTime.substring(0, 16)
                );
            }
        });
    },
    ...
    changeOverHrOrgUnit: function () {
        var that = this;
        var overStartTime = atsMlUtile.getFieldOriginalValue(
            "entries_beginTime"
        );
        if (overStartTime) {
    ...
    getSetIsCtrlHalfDayOff: function (policyId) {
        ...
        if(policyId == undefined || policyId==''|| policyId==null){
            policyId=$("#entries_policy_el").val();
        }
        var personId = $("#entries_person_el").val();
        if (!personId) {
            return;
    ...
    bfHolidayProcess: function (info) {
            ...
            if (!$(this).val()) {
                ...
                $(
                    '<label for="entries_reason" generated="true" class="error" style="display: block;">' +
                        ...
                        '</label>"'
                ).insertAfter($(this).closest(".field-ctrl").children()[0]);
            }
        });
        ...
        $("#entries_beginTime")
            ...
            .prev()
            .find(".field-label")
            .prop("title", jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_65)
            .html(jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_65);
        $("#entries_endTime")
            .parents(".field-ctrl")
            .prev()
            .find(".field-label")
            .prop("title", jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_66)
            .html(jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_66);
    },
    setTimeBegin: function () {
        if (beginTime && beginTime != "") {
            if ($("#entries_beginTime-time")) {
                $("#entries_beginTime-time").val(beginTime);
            }
            $("#entries_beginTime").val(beginTime);
        } else if (_startTimes != "") {
            if (beginTimeTimePart && beginTimeTimePart != "") {
                $("#entries_beginTime-time").val(
                    _startTimes +" " +beginTimeTimePart.substr(2)
                );
                ...
                $("#entries_beginTime-time").val(
                    _startTimes + " " + _defaultAmBeginTime
                );
                $("#entries_beginTime").val(
                    _startTimes + " " + _defaultAmBeginTime
                );
        ...
        if (endTime && endTime != "") {
            if ($("#entries_endTime-time")) {
                $("#entries_endTime-time").val(endTime);
            }
            $("#entries_endTime").val(endTime);
        } else if (_endTimes != "") {
            if (endTimeTimePart && endTimeTimePart != "") {
                $("#entries_endTime-time").val(
                    _endTimes +" " + endTimeTimePart.substr(2)
                );
                $("#entries_endTime").val(
                    _endTimes +" " + endTimeTimePart.substr(2)
                );
                ...
                $("#entries_endTime-time").val(
                    _endTimes + " " + _defaultPmEndTime
                );
                $("#entries_endTime").val(_endTimes + " " + _defaultPmEndTime);
            }
        }
    },
    setTimeStart: function () {
        if (_startTimes != "") {
            $("#entries_beginTime-time").val(_startTimes);
            if (beginTimeTimePart && beginTimeTimePart != "") {
                $("#entries_beginTime-ap").val(beginTimeTimePart);
                $("#entries_beginTime-ap_el").val(
                    beginTimeTimePart.substring(3, 8)
                );
            } else {
                $("#entries_beginTime-ap").val("上午 " + _defaultAmBeginTime);
                $("#entries_beginTime-ap_el").val(_defaultAmBeginTime);
            }
            $("#entries_beginTime").val(_startTimes);
        } else if (beginTime != "") {
            var tempStartTimes = beginTime.split(" ")[0];
            var tempStartTimesAp = beginTime.split(" ")[1];
            $("#entries_beginTime-time").val(tempStartTimes);
            if (tempStartTimesAp && tempStartTimesAp.split(":")[0] > 12) {
                $("#entries_beginTime-ap").val("下午 " + tempStartTimesAp);
                $("#entries_beginTime-ap_el").val(tempStartTimesAp);
            } else {
                $("#entries_beginTime-ap").val("上午 " + tempStartTimesAp);
                $("#entries_beginTime-ap_el").val(tempStartTimesAp);
            }
            $("#entries_beginTime").val(_startTimes);
        }
        if (_endTimes != "") {
            $("#entries_endTime-time").val(_endTimes);
            if (endTimeTimePart && endTimeTimePart != "") {
                $("#entries_endTime-ap").val(endTimeTimePart);
                $("#entries_endTime-ap_el").val(
                    endTimeTimePart.substring(3, 8)
                );
            } else {
                $("#entries_endTime-ap").val("下午 " + _defaultPmEndTime);
                $("#entries_endTime-ap_el").val(_defaultPmEndTime);
            }
            $("#entries_endTime").val(_endTimes);
        } else if (endTime != "") {
            var tempStartTimes = endTime.split(" ")[0];
            var tempStartTimesAp = endTime.split(" ")[1];
            $("#entries_endTime-time").val(tempStartTimes);
            if (tempStartTimesAp && tempStartTimesAp.split(":")[0] > 12) {
                $("#entries_endTime-ap").val("下午 " + tempStartTimesAp);
                $("#entries_endTime-ap_el").val(tempStartTimesAp);
            } else {
                $("#entries_endTime-ap").val("上午 " + tempStartTimesAp);
                $("#entries_endTime-ap_el").val(tempStartTimesAp);
            }
            $("#entries_endTime").val(tempStartTimes);
        }
    },
    tempSaveTime: function () {
        if ($("#entries_beginTime-time").val()) {
            _startTimes = $("#entries_beginTime-time").val();
            beginTimeTimePart = $("#entries_beginTime-ap").val();
            // $("#entries.beginTime").val(_startTimes)
            _endTimes = $("#entries_endTime-time").val();
            endTimeTimePart = $("#entries_endTime-ap").val();
            // $("#entries.endTime").val(_endTimes)
            beginTime = "";
            endTime = "";
        } else {
            beginTime = $("#entries_beginTime").val();
            // $("#entries.beginTime").val(beginTime)
            endTime = $("#entries_endTime").val();
            // $("#entries.endTime").val(endTime)
            if (!beginTime) {
                beginTime = "";
            }
            if (!endTime) {
    ...
    setHalfDayOff: function (info) {
        ...
        $("#entries_endTime")
            ...
            .prop("title", jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_66);
        if (
            bfHolidayTypeId == holidayTypeId &&
            bfTypeId != null &&
            bfTypeId != "" &&
            bfTypeId != customizeBF
        ) {
            $("#entries_beginTime")
                .closest(".field-ctrl")
                .eq(0)
            ...
            if (that.getOperateState() == "ADDNEW" || that.getOperateState() == "EDIT" ) {
                $("#entries_beginTime")
                    ...
                    .eq(0)
                    .html(
                        '<div id="entries_beginTime" ctrlrole="datetimepicker" validate="{required:true,myDtHrMiVldt:true}">' +
                ...
                $("#entries_endTime")
                    ...
                    .eq(0)
                    .html(
                        '<div id="entries_endTime" ctrlrole="datetimepicker" validate="{required:true,myDtHrMiVldt:true}">' +
                            '<div class="dataSpan-time">' +
                            '<div class="ui-datepicker-layout">' +
                ...
                $("#entries_beginTime-time").shrDateTimePicker("option", {
                    showTimeZoneForCtrl: false
                });
                $("#entries_beginTime-time").shrDateTimePicker(
                    "setValue",
                    $("#entries_beginTime-time").val()
                );
                $("#entries_endTime-time").shrDateTimePicker("option", {
                    showTimeZoneForCtrl: false
                });
                $("#entries_endTime-time").shrDateTimePicker(
                    "setValue",
                    $("#entries_endTime-time").val()
                );
                $($("#entries_beginTime-ap_down li")[0])
                    .val(_defaultAmBeginTime)
                    .data("value", _defaultAmBeginTime)
                    .html(
                        "<a class=''>" +
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_41 +
                            _defaultAmBeginTime +
                ...
                $($("#entries_beginTime-ap_down li")[1])
                    .val(_defaultPmBeginTime)
                    .data("value", _defaultPmBeginTime)
                    .html(
                        "<a class=''>" +
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_57 +
                            _defaultPmBeginTime +
                ...
                $($("#entries_endTime-ap_down li")[0])
                    .val(_defaultAmEndTime)
                    .data("value", _defaultAmEndTime)
                    .html(
                        "<a class=''>" +
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_41 +
                            _defaultAmEndTime +
                ...
                $($("#entries_endTime-ap_down li")[1])
                    .val(_defaultPmEndTime)
                    .data("value", _defaultPmEndTime)
                    .html(
                        "<a class=''>" +
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_57 +
                            _defaultPmEndTime +
            ...
            $("#entries_beginTime,#entries_endTime").focus(function () {
                ...
                $("#entries_endTime").attr("autocomplete", "off");
                $("#entries_beginTime").attr("autocomplete", "off");
            });
            that.setTimeBegin();
        }
        //编辑界面 设置请假开始结束时间
            ...
            that.remoteCall({
                ...
                success: function (res) {
                    ...
                    if (!that.isHalfDayOff) {
                        atsMlUtile.setTransDateTimeValue(
                            "entries_beginTime",
                            _viewBeginLeaveTime.substring(0, 16)
                        );
                        atsMlUtile.setTransDateTimeValue(
                            "entries_endTime",
                            _viewEndLeaveTime.substring(0, 16)
                        );
                    }
                    $("#entries_beginTime-time_el").val(
                        _viewBeginLeaveTime.substring(0, 10)
                    );
                    $("#entries_endTime-time_el").val(
                        _viewEndLeaveTime.substring(0, 10)
                    );
                    $("#entries_beginTime-time").val(
                        _viewBeginLeaveTime.substring(0, 10)
                    );
                    ...
                    $("#entries_beginTime-ap_el").val(
                        _viewBeginLeaveTime.substring(11, 16)
                    );
                    $("#entries_endTime-ap_el").val(
                        _viewEndLeaveTime.substring(11, 16)
                    );

                    var preBeginTime =
                        _viewBeginLeaveTime.substring(11, 13) <= 12
                            ? jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_41
                            : jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_57;
                    var preEndTime =
                        _viewEndLeaveTime.substring(11, 13) <= 12
                            ? jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_41
                            : jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_57;
                    $("#entries_beginTime-ap").val(
                        preBeginTime + _viewBeginLeaveTime.substring(11, 16)
                    );
                    $("#entries_endTime-ap").val(
                        preEndTime + _viewEndLeaveTime.substring(11, 16)
                    );
                }
            });
            that.getRemainLimit();
    ...
    setNumberFieldEnable: function () {
        var that = this;
        if (
            that.getOperateState().toUpperCase() == "EDIT" ||
            that.getOperateState().toUpperCase() == "ADDNEW"
        ) {
            var leaveBillNumberFieldCanEdit =
                that.initData.leaveBillNumberFieldCanEdit;
            if (
                typeof leaveBillNumberFieldCanEdit != "undefined" &&
                !leaveBillNumberFieldCanEdit
            ) {
                that.getField("number").shrTextField(
                    "option",
                    "readonly",
                    true
                );
        ...
        if (that.getOperateState() != "VIEW") {
            $("#entries_beginTime-time").change(function () {
                if (
                    that._unitType == 3 ||
                    $("#entries_remark").val() ==
                        jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_13 ||
                    that._unitType == 4 ||
                    $("#entries_remark").val() ==
                        jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_10
                ) {
                    $("#entries_beginTime-ap").val("");
                    $("#entries_beginTime-ap_el").val("");
                } else {
                    ...
                    if (!$("#entries_endTime-time").val()) {
                        ...
                        new Date(entrits_beginTime).getTime() >
                        new Date($("#entries_endTime-time").val()).getTime()
                    ) {
                        $("#entries_endTime-time").val(entrits_beginTime);
                        $("#entries_endTime-ap").val(
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_57 +
            ...
            $("#entries_endTime-time").change(function () {
                if (
                    ...
                    if (!$("#entries_beginTime-time").val()) {
                        ...
                        new Date(entries_endTime).getTime() <
                        new Date($("#entries_beginTime-time").val()).getTime()
                    ) {
                        $("#entries_beginTime-time").val(entries_endTime);
                        $("#entries_beginTime-ap").val(
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_41 +
            ...
            $("#entries_beginTime-ap").change(function () {
                $("#entries_beginTime-ap_el").val(
                    $("#entries_beginTime-ap").val().substring(2)
                );
            });
            $("#entries_endTime-ap").change(function () {
                $("#entries_endTime-ap_el").val(
                    $("#entries_endTime-ap").val().substring(2)
                );
            });
        }
    },
    ...
    submitEffectAction: function (event) {
        ...
        if (_self.validate() && $form.valid() && _self.verify("submit")) {
            ...
            var beginDate = atsMlUtile
                .getFieldOriginalValue("entries_beginTime")
                .split(" ")[0];
            var endDate = atsMlUtile
                .getFieldOriginalValue("entries_endTime")
                .split(" ")[0];
            var personId = $("#entries_person_el").val();
            var billType = "leave";
            _self.remoteCall({
                ...
                success: function (res) {
                    ...
                    if (result == "") {
                        if (shr.atsBillUtil.isInWorkFlow(_self.billId)) {
                            shr.showConfirm(
                                jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_14,
                                function () {
                            ...
                            shr.showConfirm(
                                jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_22,
                                function () {
                                    //start 如果是否启动半天假为是  处理上下午的请假时间 合成为时间
                                    if (_isHalfDayOff) {
                                        var halfTime = _self.getHalfTime();
                        ...
                        shr.showConfirm(
                            result +
                                jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_50,
                            function () {
                                if (
                                    shr.atsBillUtil.isInWorkFlow(_self.billId)
                                ) {
                                    shr.showConfirm(
                                        jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_14,
                                        function () {
    ...
    submitAction: function (event) {
        ...
        if (
            ...
            if (
                undefined != personId &&
                undefined != proposerId &&
                personId != "" &&
                proposerId != "" &&
                personId != proposerId
            ) {
                shr.showError({
                    message: jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_62,
                    hiddenAfter: 5
                });
                return;
            }
        ...
        if (_self.validate() && _self.verify("submit")) {
            ...
            _self.remoteCall({
                ...
                success: function (res) {
                    ...
                    if (result == "") {
                        shr.showConfirm(
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_21,
                            function () {
                                //start 如果是否启动半天假为是  处理上下午的请假时间 合成为时间
                                if (_self.getOperateState() != "VIEW") {
                                    if (_isHalfDayOff) {
                                        atsMlUtile.setTransDateTimeValue(
                                            "entries_beginTime",
                                            atsMlUtile.getFieldOriginalValue(
                                                "entries_beginTime-time"
                                            ) +
                                                " " +
                                                atsMlUtile.getFieldOriginalValue(
                                                    "entries_beginTime-ap_el"
                                                )
                                        );
                                        atsMlUtile.setTransDateTimeValue(
                                            "entries_endTime",
                                            atsMlUtile.getFieldOriginalValue(
                                                "entries_endTime-time"
                                            ) +
                                                " " +
                                                atsMlUtile.getFieldOriginalValue(
                                                    "entries_endTime-ap_el"
                                                )
                                        );
                                    }
                                }
    ...
    assembleSaveData: function (action) {
        ...
        var model;
        if (_self.getOperateState() != "VIEW") {
            model = _self.assembleModel();
        } else {
            model = shr.assembleModel(
                _self.fields,
                _self.getWorkarea(),
                _self.uuid
            );
            ...
            var entrie = {
                ...
                beginTime: atsMlUtile.getFieldOriginalValue(
                    "entries_beginTime"
                ),
                endTime: atsMlUtile.getFieldOriginalValue("entries_endTime"),

                leaveLength: atsMlUtile
                    .getFieldOriginalValue("entries_leaveLength")
                    .replace(/[^0-9.]/gi, ""),
                person: $("#entries_person").val(),
                policy: $("#entries_policy").val(),
                realBeginTime: atsMlUtile.getFieldOriginalValue(
                    "entries_realBeginTime"
                ),
                realEndTime: atsMlUtile.getFieldOriginalValue(
                    "entries_realEndTime"
                ),
                realLeaveLength: atsMlUtile.getFieldOriginalValue(
                    "entries_realLeaveLength"
                ),
        ...
        var date = model.entries.beginTime;
        ...
        _self.remoteCall({
            ...
            success: function (res) {
                ...
                if (res[personDateStr] && res[personDateStr].adminOrgUnit) {
                    model.entries["adminOrgUnit"] =
                        res[personDateStr].adminOrgUnit;
        ...
        var billmodel = JSON.parse(data.model);
        billmodel.entries.beginTime = billmodel.entries.beginTime.replace("undefined","00:00:00");
        billmodel.entries.endTime = billmodel.entries.endTime.replace("undefined","23:59:59");
        billmodel.entries.realBeginTime = billmodel.entries.beginTime;
        billmodel.entries.realEndTime = billmodel.entries.endTime;
        data.model = shr.toJSON(billmodel);
        // relatedFieldId
        ...
        if (
            jsonModel.entries.bFType != null &&
            jsonModel.entries.bFType != customizeBF &&
            jsonModel.entries.bFType != "" &&
            $("#entries_policy_holidayType_el").val() == bfHolidayTypeId
        ) {
            ...
            jsonModel.entries.beginTime = jsonModel.entries.beginTime
                .replace(".", "-")
                .replace(".", "-");
            jsonModel.entries.endTime = jsonModel.entries.endTime
                .replace(".", "-")
                .replace(".", "-");
            data.model = JSON.stringify(jsonModel);
        }
        ...
        jQuery.validator.addMethod(
            ...
            function (value, element) {
	    	    ...
	    	    * */
                ...
                if (
                    /[0-5][0-9]:[0-5][0-9]/.test(vt) ||
                    /[0-5][0-9]:[0-5][0-9]:[0-5][0-9]/.test(vt)
                ) {
    ...
    myDefaultValidate: function () {
        $("#entries_endTime,#entries_beginTime").attr(
            "validate",
            "{required:true,myDtHrMiVldt:true}"
        );
    },
                        ...
                        if (info.hasOwnProperty("holidayFileHis.id")) {
                            ...
                            that.remoteCall({
                                ...
                                success: function (res) {
                                    ...
                                    $("#entries_person_number").val(""); //@
                                    $("#entries_adminOrgUnit").shrPromptBox(
                                        "setValue",
                                        null
                                    ); //清空行政组织
                                    $("#entries_person_number").val(
                                    ...
                                    $(
                                        "#entries_adminOrgUnit"
                                    ).shrPromptBox("setValue", {
                                        id: res.adminOrgUnitId,
                                        name: res.adminOrgUnitDisplayName
                                    });
                                    that.reloadTimeAttendanceType(value);
                                    that.isLeaveLengthEdit();
    ...
    reloadTimeAttendanceType: function (res) {
        var that = this;
        if ($("#bill_flag").val() == "commissioner") {
            that.loadTimeAttendanceType(res);
        }
    },
    ...
    changeBFType: function (value) {
        ...
        if (value.current == null || value.current.id == customizeBF) {
            ...
            if (that.isHalfDayOff) {
                $("#entries_beginTime")
                    ...
                    .eq(0)
                    .html(
                        '<div id="entries_beginTime" ctrlrole="dateSpanPicker" validate="{required:true,myDtHrMiVldt:true}">' +
                            '<div class="dataSpan-time">' +
                            '<div class="ui-datepicker-layout">' +
                            '<input id="entries_beginTime-time" class="input-height" type="text" validate="{required:true}" name="entries.beginTime-time" ctrlrole="datepicker">' +
                            "</div>" +
                ...
                $("#entries_endTime")
                    ...
                    .eq(0)
                    .html(
                        '<div id="entries_endTime" ctrlrole="dateSpanPicker" validate="{required:true,myDtHrMiVldt:true}">' +
                            '<div class="dataSpan-time">' +
                            '<div class="ui-datepicker-layout">' +
                            '<input id="entries_endTime-time" class="input-height" type="text" validate="{required:true}" name="entries.endTime-time" ctrlrole="datepicker">' +
                            "</div>" +
                ...
                $("#entries_beginTime,#entries_endTime").blur(function () {
                    ...
                    if (value == "" || value == null || value.trim() == "") {
                        ...
                        $(
                            ...
                            $(this).closest(".field-ctrl").children()[0]
                        );
                    }
                });
        ...
        that.remoteCall({
            ...
            success: function (res) {
                if (
                    res.maternityEndTime != null &&
                    res.maternityEndTime != ""
                ) {
    ...
    bfTimeChange: function () {
        ...
        $("#entries_mLEndTime").change(function () {
            var mlEndDate = new Date(
                atsMlUtile.getFieldOriginalValue("entries_mLEndTime")
            );
            ...
            $("#entries_beginTime").shrDateTimePicker(
                "setValue",
        ...
        $("#entries_childbirthday").change(function () {
            var childbirthday = new Date(
                atsMlUtile.getFieldOriginalValue("entries_childbirthday")
            );
            var date = new Date(
                childbirthday.setFullYear(childbirthday.getFullYear() + 1)
            );
            ...
            $("#entries_endTime").shrDateTimePicker(
                "setValue",
                date.getFullYear() + "-" + month + "-" + day
            );
            //计算请假时长
    ...
    initDialog: function () {
        ...
        if (
            ...
            parent.window.shr.getCurrentViewPage().uipk !=
                "com.kingdee.eas.hr.ats.app.WorkCalendar.empATSDeskTop" &&
            parent.window.shr.getCurrentViewPage().uipk !=
                "com.kingdee.eas.hr.ats.app.WorkCalendarItem.listSelf"
        ) {
            $("#message_info").hide();
        } else {
        ...
        if (
            ...
            $("#showMoreLeaveInfo").click(function (e) {
                ...
                if (personId != null && personId != "") {
                    ...
                    shr.showError({
                        message:
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_16
                    });
                }
            });
        }
        ...
        that.remoteCall({
            ...
            success: function (res) {
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    var typeName =
                        leaveBillColls[i].entries[0].policy.holidayType.name;
                    var length = "";
                    if (
                        typeof leaveBillColls[i].entries[0].leaveLength !=
                        "undefined"
                    ) {
                    ...
                    var unitType = "";
                    if (
                        typeof leaveBillColls[i].entries[0].remark !=
                        "undefined"
                    ) {
                    ...
                    var reason = "";
                    if (
                        typeof leaveBillColls[i].entries[0].reason !=
                        "undefined"
                    ) {
                    ...
                    var rowi = {};
                    rowi.id = i + 1; //编号
                    rowi.beginTime = atsMlUtile.dateTimeToUserFormat(beginTime); //开始时间
                    rowi.endTime = atsMlUtile.dateTimeToUserFormat(endTime); //结束时间
                    rowi.typeName = typeName; //事假
                    rowi.length = atsMlUtile.numberToUserFormat(length); //1
                    rowi.unitType = unitType; //天
                    rowi.reason = reason; //原因
                    rowi.state = state; //审批状态
                    rowi.applyDate = atsMlUtile.dateToUserFormat(applyDate); //申请日期
                    //rowi.name="5555";
    ...
    isLeaveLengthEdit: function () {
        ...
        if (that.getOperateState() != "VIEW") {
            ...
            if (personId == null || personId == "" || personId == undefined) {
                shr.showWarning({
                    message: jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_38
                });
                return;
            }
            that.remoteCall({
     ...
     */
    loadTimeAttendanceType: function (res) {
        var that = this;
        ...
        if (that.getOperateState() != "VIEW") {
            ...
            if (that.getOperateState() != "ADDNEW") {
                ...
                endTime = $("#entries_endTime-time").val()
                    ? $("#entries_endTime-time").val()
                    : (endTime = atsMlUtile.getFieldOriginalValue(
                          "entries_endTime"
                      ));
            } else {
                beginTime = $("#entries_beginTime-time").val();
            ...
            if (res != null && res.current) {
                if (beginTime == null || beginTime == "") {
                    beginTime = res.current["holidayFileHis.EFFDT"];
                }
                if (endTime == null || endTime == "") {
                    endTime = res.current["holidayFileHis.LEFFDT"];
                }
            }
    ...
    setAttendTypeDefault: function (attendColl, size, unitTypeName) {
        ...
        if (that.getOperateState() == "ADDNEW") {
            for (var i = 0; i < size; i++) {
                if (
                    attendColl[i].holidayType.id == attendTypeIdId ||
                    i == attendTypeIdId
                ) {
            ...
            if (temp == 0) {
                ...
                shr.showWarning({
                    message: jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_39
                });
            }
        }
    },
        ...
        that.remoteCall({
            ...
            success: function (res) {
                ...
                if ($("#breadcrumb").attr("id") != "breadcrumb") {
                    if (
                        $("#entries_person").attr("title") == null ||
                    ...
                    if ($("#bill_flag").val() != "commissioner") {
                        personShowName = info.personName; //jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_20;
                ...
                if (leaveDate != "") {
                    ...
                    if (leaveTimes == 0) {
                        ...
                        var mes =
                            ...
                            "：" +
                            shr.formatMsg(
                                jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_7,
                                [
                                    personShowName,
                                    "<font id='leaveDes'>",
                                    leaveTimes,
                                    "</font>，",
                                    "<font id='leaveDes_Time'> " +
                                        leaveDate +
                                        "</font> "
                                ]
                            ); //+
                        ...
                        info.leaveAuditMidTimes > 0 ||
                        info.leaveNotAuditTimes > 0
                    ) {
                        //info.data.personName
                        var mes =
                            "&nbsp;&nbsp; " +
                        ...
                        var mes =
                            ...
                            "：" +
                            shr.formatMsg(
                                jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_6,
                                [
                                    //"全部审核通过，" +
                                    personShowName,
                                    "<font id='leaveDes'>",
                    ...
                    var mes =
                        ...
                        "： " +
                        shr.formatMsg(
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_5,
                            [
                                personShowName,
                                "<font id='leaveDes'>",
                                info.leaveTimes
        ...
        if (that.isFromWF()) {
            ...
            that.remoteCall({
                ...
                success: function (res) {
                    ...
                    if (message == "TRUE") {
                        var info = that.getRealAndRemainLimit(
                            personId,
                            holidayPolicyId,
                            billId
                        );
                        ...
                        $("#entries_policy").append(
                            "(" +
                                ...
                                info.remainLimitWithoutLeaveLength +
                                "" +
                                info.unit +
                                "," +
        ...
        that.remoteCall({
            ...
            success: function (res) {
                ...
                if ($("#breadcrumb").attr("id") != "breadcrumb") {
                    ...
                    if ($("#bill_flag").val() != "commissioner") {
                        personShowName =
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_20;
                    } else if ($("#bill_flag").val() == "commissioner") {
                        if (
                            $("#entries_person").attr("title") == null ||
                            $("#entries_person").attr("title") == ""
    ...
    setButtonVisible: function () {
        ...
        if (
            this.getOperateState().toUpperCase() == "ADDNEW" ||
            this.getOperateState().toUpperCase() == "EDIT"
        ) {
            $("#returnToLeaveBillList").hide();
        }
    },
    ...
    showCancelLeaveBillInfo: function (res) {
        ...
        for (var i = 0; i < size; i++) {
		    ...
		    */
            ...
            html +=
                '<span class="field_input">' +
                infoColl[i].bill.applyDate.substring(0, 10) +
                "</span>";
            html += "</div>";
            html += '<div class="col-lg-2 field-desc"></div>';
            html += "</div>";
    ...
    leaveTimeChangeDealOfDay: function () {
        ...
        $("#entries_beginTime").on("change", function () {
            ...
            beginTimer = setTimeout(function () {
                ...
                if (
                    $("input[id=entries_endTime-time]").val() ||
                    $("input[id=entries_endTime]").val()
                ) {
                    that.getRealLeaveLengthOfDay();
                }
            }, 30);
        ...
        $("#entries_endTime").on("change", function () {
            ...
            timer = setTimeout(function () {
                ...
                if (
                    $("input[id=entries_beginTime-time]").val() ||
                    $("input[id=entries_beginTime]").val()
                ) {
                    that.getRealLeaveLengthOfDay();
                }
            }, 30);
        ...
        $("#entries_beginTime-ap").change(function () {
            ...
            apbeginTimer = setTimeout(function () {
                holidayFileHisId = "";
                if (
                    atsMlUtile.getFieldOriginalValue("entries_beginTime") ||
                    atsMlUtile.getFieldOriginalValue("entries_endTime")
                ) {
                    that.getRealLeaveLengthOfDay();
                }
            }, 30);
        ...
        $("#entries_endTime-time").change(function () {
            ...
            apbeginTimer = setTimeout(function () {
                holidayFileHisId = "";
                if (
                    atsMlUtile.getFieldOriginalValue("entries_endTime-time") ||
                    atsMlUtile.getFieldOriginalValue("entries_beginTime-time")
                ) {
                    that.getRealLeaveLengthOfDay();
                }
            }, 30);
	...
	*/,
    getRemainLimit: function () {
        ...
        if (hrOrgUnitId == null || hrOrgUnitId == "") {
            ...
            if (hrOrgUnitId == null || hrOrgUnitId == "") {
                shr.showWarning({
                    message: jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_35
                });
                return;
            }
        }
        ...
        if (personId == null || personId == "") {
            shr.showWarning({
                message: jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_37
            });
            return;
        }
        ...
        if (res) {
            ...
            for (var i = 0; i < $('div[id^="div"]').size(); i++) {
                    ...
                    for (var prop in vacationRemain) {
                        ...
                        if (indexNum > -1) {
                            ...
                            if (vacationRemain[prop] == "") {
                                ...
                                var numberOptions = {
                                    decimalPrecision: res.saveLengthMap[prop]
                                };
                ...
                if (
                    !hasProp &&
                    this.uipk !=
                        "com.kingdee.eas.hr.ats.app.AtsLeaveBillFormWorkflow"
                ) {
                    if (fontObject.length != 0) {
                        var newString = "";
                        ...
                        var newString = "";
                        divObject.append(
                            "<font class='remain_info'>" + newString + "</font>"
                        );
                    }
                }
            }
    ...
    calculateLeaveLengthSub: function () {
        ...
        if (
            $("#entries_beginTime-time").val() != "" &&
            $("#entries_beginTime-time").val() != null &&
            $("#entries_endTime-time").val() != "" &&
            $("#entries_endTime-time").val() != null &&
            $("#entries_beginTime-ap").val() != "" &&
            $("#entries_beginTime-ap").val() != null &&
            $("#entries_endTime-ap").val() != "" &&
            $("#entries_endTime-ap").val() != null
        ) {
            var regEx = new RegExp("\\-", "gi");
            begintime = begintime.replace(regEx, "/");
    ...
    getRealLeaveLengthOfDay: function () {
        ...
        var holidayTypeId = $("#entries_policy_holidayType_el").val(); //假期分类ID
        var leaveLength = parseFloat(
            atsMlUtile.getFieldOriginalValue("entries_leaveLength")
        );
        ...
        if (
            ...
            that.remoteCall({
                ...
                param: {
                    ...
                    holidayTypeId: holidayTypeId,
                    hrOrgUnitId: hrOrgUnitId,
                    isElasticCalLen: $("#entries_isElasticCalLen").shrCheckbox(
                        "isSelected"
                    )
                },
                success: function (res) {
        ...
        //半天假
            ...
            beginDate = atsMlUtile.getFieldOriginalValue(
                "entries_beginTime-time"
            );
            ...
            if (
                ...
                if (
                    $("#entries_beginTime-ap")
                        .val()
                        .indexOf(_defaultAmBeginTime) > 0
                ) {
                    beginTime =
                        jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_40;
                } else {
                    beginTime =
                        jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_56;
                }
                if (
                    $("#entries_endTime-ap").val().indexOf(_defaultAmEndTime) >
                    0
                ) {
            ...
            if (
                atsMlUtile.getFieldOriginalValue("entries_beginTime") != "" &&
                atsMlUtile.getFieldOriginalValue("entries_beginTime") != null &&
                atsMlUtile.getFieldOriginalValue("entries_endTime") != "" &&
                atsMlUtile.getFieldOriginalValue("entries_endTime") != null
            ) {
                beginTime = atsMlUtile.getFieldOriginalValue(
                    "entries_beginTime"
                );
                ...
                that.remoteCall({
                    ...
                    param: {
                        ...
                        hrOrgUnitId: hrOrgUnitId,
                        isElasticCalLen: $(
                            "#entries_isElasticCalLen"
                        ).shrCheckbox("isSelected")
                    },
                    success: function (res) {
                        info = res;
                        if (info.errorString) {
        ...
        if (
            ...
            that.remoteCall({
                ...
                success: function (res) {
                    info = res;
                    var day =
                        parseFloat(info.leaveBillLength) /
                        1000.0 /
                        60 /
                        60 /
                        24;
    ...
    verify: function (actionType) {
        ...
        if (that.getOperateState() == "VIEW") {
            ...
            var beginTime = that.getSplitString(
                atsMlUtile.getFieldOriginalValue("entries_beginTime")
            );
            var endTime = that.getSplitString(
                atsMlUtile.getFieldOriginalValue("entries_endTime")
            );
            var personId = $("#entries_person").val();
            var leaveLength = atsMlUtile
                .getFieldOriginalValue("entries_leaveLength")
                .replace(/[^0-9.]/gi, "");
            var regEx = new RegExp("\\/", "gi");
            var billId = $("#id").val();
            ...
            //前置假校验
            info_res = that.validatePreHolidayType(
                personId,
                beginTime,
                endTime,
                holidayTypeId
            ...
            var info_res = that.validateHolidayPolicyControl(
                personId,
                holidayTypeId,
                beginTime,
                endTime,
        ...
        if (holidayTypeId == bfHolidayTypeId) {
            if ($("#entries_bFType_el").val() == "") {
                shr.showInfo({
                    message: jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_67
                });
                return false;
            } else if ($("#entries_bFType_el").val() != customizeBF) {
                var childbirthday = atsMlUtile.getFieldOriginalValue(
                    "entries_childbirthday"
                );
                var mlEndTime = atsMlUtile.getFieldOriginalValue(
                    "entries_mLEndTime"
                );
                var beginTime = atsMlUtile.getFieldOriginalValue(
                    "entries_beginTime"
                );
                var endTime = atsMlUtile.getFieldOriginalValue(
                    "entries_endTime"
                );
                if (mlEndTime != null && mlEndTime != "") {
                    if (beginTime != null && beginTime != "") {
                        ...
                        if (date1 >= date3) {
                            shr.showInfo({
                                message:
                                    jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_68
                            });
                            return false;
                        }
                    }
                ...
                if (childbirthday != null && childbirthday != "") {
                    if (endTime != null && endTime != "") {
                        ...
                        if (date1 < date2) {
                            shr.showInfo({
                                message:
                                    jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_69
                            });
                            return false;
                        }
                    }
                    if (beginTime != null && beginTime != "") {
                        ...
                        if (date1 >= date3) {
                            shr.showInfo({
                                message:
                                    jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_70
                            });
                            return false;
                        }
                    }
        ...
        if (existHolidayType == "" || existHolidayType == null) {
            shr.showInfo({
                message: jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_36
            });
            return false;
        }
        ...
        if (
            ...
            beginTime =
                atsMlUtile.getFieldOriginalValue("entries_beginTime-time") +
                " " +
                atsMlUtile.getFieldOriginalValue("entries_beginTime-ap_el");
            endTime =
                atsMlUtile.getFieldOriginalValue("entries_endTime-time") +
                " " +
                atsMlUtile.getFieldOriginalValue("entries_endTime-ap_el");
        } else {
            beginTime = atsMlUtile.getFieldOriginalValue("entries_beginTime");
            endTime = atsMlUtile.getFieldOriginalValue("entries_endTime");
        }
        ...
        if (leaveLength <= 0) {
            shr.showWarning({
                message: jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_33
            });
            return false;
        }
        if (longTime > 365 * 24 * 60 * 60 * 1000) {
            shr.showWarning({
                message: jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_32
            });
            return false;
        }
        if (
            ...
            if (
                realLengthInfo.leaveBillDays + realLengthInfo.segRest <
                    leaveLength &&
                (that._unitType == 1 || $("#entries_remark").val() == "天")
            ) {
                shr.showError({
                    message:
                        "请假时间不能超过" +
                        realLengthInfo.wholeLen.toFixed(
                ...
                realLengthInfo.leaveBillDays + realLengthInfo.segRest <
                    leaveLength &&
                (that._unitType == 0 || $("#entries_remark").val() == "小时")
            ) {
                shr.showError({
                    message:
                        "请假时间不能超过" +
        ...
        //启动半天假
            ...
            // 时间 通过排班 工作日历 计算后的实际时间
            var realLengthInfo;
            var beginDateTemp = atsMlUtile.getFieldOriginalValue(
                "entries_beginTime-time"
            );
            var endDateTemp = atsMlUtile.getFieldOriginalValue(
                "entries_endTime-time"
            );
            ...
            if (hrOrgUnitId == null || hrOrgUnitId == "") {
                ...
                if (hrOrgUnitId == null || hrOrgUnitId == "") {
                    shr.showWarning({
                        message:
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_35
                    });
                    return;
                }
            }
            ...
            if (
                $("#entries_beginTime-ap").val().indexOf(_defaultAmBeginTime) >
                0
            ) {
                beginTimeTemp =
                    jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_40;
                ...
                if (
                    ...
                    shr.showError({
                        message: shr.formatMsg(
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_30,
                            [realLengthInfo.leaveBillDays]
                        )
                    });
                    return false;
                } else if (
                    ...
                    shr.showError({
                        message: shr.formatMsg(
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_31,
                            [realLengthInfo.leaveBillDays]
                        )
                    });
                    return false;
        ...
        billId = $("#id").val();
        var info_res = that.validateLeaveBillCycleNew(
            personId,
            beginTime,
            endTime,
            holidayTypeId,
        ...
        if (
            validateInfo != null &&
            validateInfo.errorString != undefined &&
            validateInfo.errorString != ""
        ) {
        ...
        if (info && info.addFlag > 0) {
            shr.showError({
                message: shr.formatMsg(
                    jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_59,
                    [
                        info.billNo,
                        info.personName,
                        info.realBeginDate,
    ...
    validateHolidayPolicyControl: function (
        personId,
        holidayTypeId,
        beginTime,
        endTime,
        ...
        if (
            holidayTypeId == bfHolidayTypeId &&
            bfTypeId != "" &&
            bfTypeId != customizeBF && bfTypeId!=undefined
        ) {
            beginTime = atsMlUtile.getFieldOriginalValue("entries_beginTime");
            endTime = atsMlUtile.getFieldOriginalValue("entries_endTime");
        }
     ...
     * 校验前置假
     */
    validatePreHolidayType: function (
        personId,
        beginTime,
        endTime,
        holidayTypeId
    ...
    //验证请假周期
    validateLeaveBillCycleNew: function (
        personId,
        beginTime,
        endTime,
        holidayTypeId,
    ...
    //验证请假周期
    validateLeaveBillCycle: function (
        personId,
        beginTime,
        endTime,
        holidayTypeId,
    ...
    calculateLeaveLength_Longon: function () {
        ...
        if (
            begintime_ap == jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_40 &&
            ...
            begintime_ap == jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_56 &&
            endtime_ap == jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_56
        ) {
        ...
        if (
            ...
            if (that._unitType == 2) {
                ...
                if (longTime < 0) {
                    shr.showInfo({
                        message:
                            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_26
                    });
                    return false;
                } else {
            ...
            } else if (that._unitType == 1) {
                //单位为天
                var begintime_day = $("#entries_beginTime-time").val();
                var endtime_day = $("#entries_endTime-time").val();
                var begintime_ap_day = $("#entries_beginTime-ap")
                    .val()
                    .substring(2); //开始时间的 上午和下午
                var endtime_ap_day = $("#entries_endTime-ap")
                    .val()
                    .substring(2); //结束时间的上午和下午
                ...
                var longTime2 =
                    endTimeDayOfDate.getTime() - beginTimeDayOfDate.getTime();
    ...
    showMoreBindClick: function () {
        ...
        dialog_Html =
            "<div id='dialogViewMore' style = 'font-size: 12px; padding: 10px; width: 93%' title=''>" +
            "<div>" +
    ...
    setNavigateLine: function () {
        ...
        if (
            fromFlag == "employeeBoard" ||
            fromFlag == "punchCard" ||
            ("fromPunchCard" == punchCardFlag &&
                "com.kingdee.eas.hr.ats.app.WorkCalendarItem.listSelf" ==
                    parentUipk) ||
            ("empolyeeBoardFlag" == empolyeeBoardFlag &&
                "com.kingdee.eas.hr.ats.app.WorkCalendar.empATSDeskTop" ==
                    parentUipk)
        ) {
            //来自我的考勤的时候。将导航条删除掉。
            ...
            window.parent.changeDialogTitle(
                jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_55
            );
        }
    },
    ...
    getCurrentModel: function () {
        var that = this;
        var model = shr.ats.AtsLeaveBillEdit.superClass.getCurrentModel.call(
            this
        );
    ...
    initElasticCalCtrl: function () {
        ...
        if (
            ...
            var personId =
                $("#entries_person_el").val() || $("#entries_person").val();
            var holidayPolicyId =
                $("#entries_policy_el").val() || $("#entries_policy").val();
            var beginTime =
                _viewBeginLeaveTime == ""
                    ? atsMlUtile.getFieldOriginalValue("entries_beginTime")
                    : _viewBeginLeaveTime;
            var endTime =
                _viewEndLeaveTime == ""
                    ? atsMlUtile.getFieldOriginalValue("entries_endTime")
                    : _viewEndLeaveTime;
            shr.atsBillUtil.showIsElasticCalCtrl(
                that,
                personId,
                holidayPolicyId,
                beginTime,
    ...
    showIsElasticCalCtrl: function () {
        ...
        var personId =
            $("#entries_person_el").val() == "" ||
            $("#entries_person_el").val() == undefined
                ? $("#entries_person").val()
                : $("#entries_person_el").val();
        var holidayPolicyId =
            $("#entries_policy_el").val() == "" ||
            $("#entries_policy_el").val() == undefined
                ? $("#entries_policy").val()
                : $("#entries_policy_el").val();
        var beginTime =
            atsMlUtile.getFieldOriginalValue("entries_beginTime") == ""
    ...
    showTips: function () {
        //此处选择器中用中文括号，页面上是中文
        $(
            '[title="' + jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_11 + '"]'
        ).append('<span id="tipsIsElasticCal"></span>');
        var tipsIsElasticCalText =
            "&nbsp;&nbsp;" +
            jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_19 +
            "<br/>";
        var tipsIsElasticCalLog =
            '<div id="tipsIsElasticCal-dialog" style="display:none;border:solid 1px gray;border-radius:5px;width:42%;height:100px;position:absolute;left:15%;z-index:1;margin-top:0px;background: aliceblue;"><div style="position:absolute"><font color="gray">' +
            tipsIsElasticCalText +
            "</font></div></div>";
        //此处选择器中用中文括号，页面上是中文
        $(
            '[title="' + jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_11 + '"]'
...
showHolidayLimit = function (proposerId, hrOrgUnitId, holidayPolicyId) {
    var url =
        shr.getContextPath() +
        "/dynamic.do?uipk=com.kingdee.eas.hr.ats.app.HolidayLimitPerself.list&flag=perself&type=day";
    url += "&proposerId=" + encodeURIComponent(proposerId);
    $("#operationDialog").remove();
    var dialogHtml =
        '<div id="operationDialog" style="overflow:hidden;display:none;">' +
...
changeColor = function (
    ...
    if (unitTypeName == "" || unitTypeValue == "") {
        shr.showInfo({
            message: jsBizMultLan.atsManager_atsLeaveBillEdit_i18n_18
        });
        return false;
    }
    $("#leaveRemark").html(
    ...
    if ($("#entries_policy_holidayType_el").val() == bfHolidayTypeId) {
        ...
        $("#entries_bFType")
            .closest(".ui-promptBox-frame")
            .css({ backgroundColor: "#d9edf7 !important" });
        $("#entries_childbirthday").parents(".field-basis1").hide();
        $("#entries_mLEndTime").parents(".field-basis1").hide();
    } else {
