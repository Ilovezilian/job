...
shr.defineClass("shr.ats.AtsLeaveBillAllList", shr.framework.List, {
	...
	initalizeDOM : function () {
		...
		shr.ats.AtsLeaveBillAllList.superClass.initalizeDOM.call(that);
		jsBinder.gridLoadComplete=function(data){
			shr.ats.AtsLeaveBillAllList.superClass.gridLoadComplete.call(that,data);
...
//			}
	...
	,againstApproveAction : function(){
		...
		var msg = "";
		var salaryMsg = "";
		var url = shr.getContextPath() + "/dynamic.do?handler=com.kingdee.shr.ats.web.handler.AtsLeaveBillListHandler&method=checkDetailSummary";
		   shr.ajax({
			type:"post",
			async:false,
			data :{billIds:billIds} ,
	...
	queryGrid: function(){
		...
		if(dateRequiredValidate){
			shr.ats.AtsLeaveBillAllList.superClass.queryGrid.call(this);
		}
	}
	/**
	...
	,addNewAction: function() {
		this.reloadPage({
			uipk: "com.kingdee.eas.hr.ats.app.AtsLeaveBillAllForm",
			method: 'addNew'
		});
	}
...
//			    url +='&uipk=com.kingdee.eas.hr.ats.app.AtsLeaveBillAllList';
//			   shr.remoteCall({
//				type:"post",
...
//	
//		var url = shr.getContextPath() + "/dynamic.do?handler=com.kingdee.shr.ats.web.handler.AtsLeaveBillEditHandler&method=uploadFile";
//		//在点击确定前，对文件进行上传处理
...
//	}
	,importFileData: function(){
		//alert("读取服务器目录文件 解析");
		var that=this;
		var url = shr.getContextPath() + "/dynamic.do?handler=com.kingdee.shr.ats.web.handler.AtsLeaveBillEditHandler&method=importFileData";
		$.ajax({
			url: url,
			beforeSend: function(){
				openLoader(1);
	...
	,exportFileData: function(){
		 var url = shr.getContextPath() + "/dynamic.do?handler=com.kingdee.shr.ats.web.handler.AtsLeaveBillListHandler&method=downloadFile";
		 location.href = url;
	}
    /**
	 * 套打模板名称
