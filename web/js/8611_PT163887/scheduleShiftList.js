shr.defineClass("shr.ats.ScheduleShiftList", shr.ats.AttendanceOrgList, {
	HORIZONTAL_SHOW:0,
	VERTICAL_SHOW:1,
	UNCOMPLETE_SHOW:2,
	rowNumPerPage: 30,
	...
	initalizeDOM : function () {
	    ...
	    shr.ats.ScheduleShiftList.superClass.initalizeDOM.call(this);
	...
	initalSearch : function(){
		...
		$('#microToolbar1').parent().css({position: 'absolute',top: '38px',right: '20px'});
		var options = {
			gridId: "dataGrid",
			...
			+'/dynamic.do?uipk=com.kingdee.eas.hr.ats.app.ScheduleShift.list&method=getProperField&paramColumnData=' + encodeURIComponent(shr.toJSON(_self.searchParam()))
		};
	...
	,importDataAction:function(){
		this.reloadPage({
			uipk: 'com.kingdee.eas.hr.ats.app.ScheduleShiftImport'
		});		
	}
	...
	batchModifyAction:function(){
		...
		if(that.billId != undefined && that.billId != null && that.billId != '' && that.showType == 0){
			...
			if( billIds.length == 1){
				...
				this.reloadPage({
					uipk: 'com.kingdee.eas.hr.ats.ScheduleShiftItemModify',
	...
	getVerticaliQueryParam : function(){
		var serviceId = shr.getUrlRequestParam("serviceId");
		var url = shr.getContextPath() + "/dynamic.do?handler=com.kingdee.shr.ats.web.handler.ScheduleShiftListHandler&method=getPersonShift";
...
//		var endDate = $('#endDate').val();
		...
		shr.ajax({
				...
				url:shr.getContextPath() + "/dynamic.do?handler=com.kingdee.shr.ats.web.handler.ScheduleShiftListHandler&method=getGridColModel"+'&serviceId='+encodeURIComponent(serviceId),
				success:function(res){
	...
	,handleMicroToolbarInfo : function (data) {
		...
		html += "<span id='gripage' class='ui-paging-info' style='cursor: default;display: inline-block;font-size: 13px;padding: 2px 5px 0 0;position: relative;top: 5px;'></span>";
	...
	,deleteAction:function() {
		...
		shr.showConfirm(jsBizMultLan.atsManager_scheduleShiftList_i18n_23, function(){
			...
			shr.doAction({
				url: shr.getContextPath() + "/dynamic.do?handler=com.kingdee.shr.ats.web.handler.ScheduleShiftListHandler&method=delete" ,
				type: 'post', 
					data: data,
	...
	,getPageState: function() {
		var that = this;
		var pageState = shr.ats.ScheduleShiftList.superClass.getPageState.call(this);
		var data = {};
	...
	,setPageState: function(pageState) {
		var that = this;
		shr.ats.ScheduleShiftList.superClass.setPageState.call(this, pageState);
	...
	exportCommonParam : function(){
		...
		var uipk = "com.kingdee.eas.hr.ats.app.ScheduleShift.list";
