...
shr.defineClass("shr.ats.CancelLeaveBillAllList", shr.framework.List, {
	...
	initalizeDOM : function () {
		...
		shr.ats.CancelLeaveBillAllList.superClass.initalizeDOM.call(this);
	//	that.initToolBars();	
		jsBinder.gridLoadComplete=function(data){
			shr.ats.CancelLeaveBillAllList.superClass.gridLoadComplete.call(that,data);
	...
	,importAction: function(){
		...
		var url = shr.getContextPath() + "/dynamic.do?handler=com.kingdee.shr.ats.bill.util.BillBizUtil";
			    url +='&uipk=com.kingdee.eas.hr.ats.app.CancelLeaveBillAllList';
			   shr.remoteCall({
				type:"post",
				method:"checkImportPermssion",
				url:url,
				success:function(res){
					shr.ats.CancelLeaveBillAllList.superClass.importAction.call(this);
				}
			});
	...
	shr.ats.CancelLeaveBillAllList.superClass.submitAction.call(this);
	}
	...
	,againstApproveAction : function(){
		...
		var msg = "";
		var salaryMsg = "";
		var url = shr.getContextPath() + "/dynamic.do?handler=com.kingdee.shr.ats.web.handler.CancelLeaveBillListHandler&method=checkDetailSummary";
		   shr.ajax({
			type:"post",
			async:false,
		...
		$('#orgFillDiv').dialog({
			title: jsBizMultLan.atsManager_cancelLeaveBillAllList_i18n_18,
			width: 900,
			height: 400,
			modal: true,
			resizable: false,
			...
			buttons: [{
					text: jsBizMultLan.atsManager_cancelLeaveBillAllList_i18n_19,
					click: function() {						
						$(this).dialog( "close" );
						var reason = $("#orgFillDiv #reason").val();
						if (reason) {		
							shr.showConfirm(jsBizMultLan.atsManager_cancelLeaveBillAllList_i18n_9, function(){
									...
									shr.remoteCall({
										type:"post",
										url:shr.getContextPath() + "/dynamic.do?uipk=com.kingdee.eas.hr.ats.app.CancelLeaveBillAllList",
										method:"againstApprove",
							...
							shr.showError({message: jsBizMultLan.atsManager_cancelLeaveBillAllList_i18n_21});
							return;
						} 
					}
				},{
					text: jsBizMultLan.atsManager_cancelLeaveBillAllList_i18n_20,
					click: function(){
						$(this).dialog( "close" );
					}
				}],
	...
	,setGridCustomParam: function() {
		...
		if (atsCalFlag){
			...
			if(fastFilterItems && fastFilterItems['entries.realLeaBeginTime_entries.realLeaEndTime'] && fastFilterItems['entries.realLeaBeginTime_entries.realLeaEndTime'].values){
				params["beginDate"] = this.getFastFilterItems()['entries.realLeaBeginTime_entries.realLeaEndTime'].values["startDate"];
				params["endDate"] = this.getFastFilterItems()['entries.realLeaBeginTime_entries.realLeaEndTime'].values["endDate"];
			} else {
	...
	,prepareSubmitEffect : function (event, action){
		...
		_self.remoteCall({
			...
			success : function(res) {
				...
				if(undefined!=res.summary){
					shr.showInfo({message:jsBizMultLan.atsManager_cancelLeaveBillAllList_i18n_12 + res.submitNum
						+ jsBizMultLan.atsManager_cancelLeaveBillAllList_i18n_0 + res.submitSuccessNum + failMsg +"--"+ res.summary});	
				}else{
					shr.showInfo({message:jsBizMultLan.atsManager_cancelLeaveBillAllList_i18n_12 + res.submitNum
						+ jsBizMultLan.atsManager_cancelLeaveBillAllList_i18n_0 + res.submitSuccessNum + failMsg });		
				}
