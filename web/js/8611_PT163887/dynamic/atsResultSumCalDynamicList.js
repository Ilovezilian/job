shr.defineClass("shr.ats.atsResultSumCalDynamicList", shr.ats.atsResultSumDynamicList, {
	initalizeDOM : function () {
		var _self = this;
		shr.ats.atsResultSumCalDynamicList.superClass.initalizeDOM.call(_self);
	...
	//全部计算/计算选中行
	 calculateCustomerSumResult: function(fids){
		...
		var url = shr.getContextPath() + "/dynamic.do?uipk=com.kingdee.eas.hr.ats.app.AttendanceResultSum.dynamicList";
		shr.remoteCall({
				type:"post", 
				async:true,
				url:url,
...
//		$('input[name=MONTH]').val(curDateM);
	...
	//已汇总页签--转薪资
  	salarySumResult: function(){
		...
		var attendPolicyId = shr.attenceCalCommon.getFilterParamValues("attencePolicy");
		 openLoader(1);
		 var url = shr.getContextPath() + "/dynamic.do?uipk=com.kingdee.eas.hr.ats.app.AttendanceResultSum.dynamicList$fragment";
		 shr.remoteCall({
				type:"post", 
				url:url, 
	...
	deleteAction: function(){
		...
		var url = shr.getContextPath() + "/dynamic.do?uipk=com.kingdee.eas.hr.ats.app.AttendanceResultSum.dynamicList&method=delete"
		if(ids.length > 0){
	...
	saveAction: function(){
		...
		var url = shr.getContextPath() + "/dynamic.do?uipk=com.kingdee.eas.hr.ats.app.AttendanceResultSum.dynamicList&method=save"
