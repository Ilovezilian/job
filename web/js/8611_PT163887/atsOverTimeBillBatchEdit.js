	...
	,checkOtType:function(){
		...
		if  (rows>0){
			for (var i=1;i<=rows;i++){
				...
				var person=$("#entries").jqGrid("getCell", tempId,"person");
				var otType = $("#entries").jqGrid("getCell", tempId,"otType"); //otDate
				var otDate = $("#entries").jqGrid("getCell", tempId,"otDate");
				if(otType == null || otType == ''){
					var errorString = shr.formatMsg(jsBizMultLan.atsManager_atsOverTimeBillBatchEdit_i18n_checkOtType_01, [person.name,otDate]);
					shr.showWarning({message: errorString});
					closeLoader();
					return false;
				}
...
//			}
	...
	,fillGrid:function(){
        ...
        var restTime = $(window.frames["orgFillDiv"].document).find("#restTime").val() ;
        var restStartTime = $(window.frames["orgFillDiv"].document).find("#restStartTime").val() ;
        var restStartTime2 = $(window.frames["orgFillDiv"].document).find("#restStartTime2").val() ;
        var restEndTime = $(window.frames["orgFillDiv"].document).find("#restEndTime").val() ;
        var restEndTime2 = $(window.frames["orgFillDiv"].document).find("#restEndTime2").val() ;
			var otReason_el =$(window.frames["orgFillDiv"].document).find("#otReason_el").val() ;
			var otReason = $(window.frames["orgFillDiv"].document).find("#otReason").val() ;
			var applyOTTime = $(window.frames["orgFillDiv"].document).find("#applyOTTime").val() ;
        	...
        	var hasNum = $("#entries").getGridParam("reccount") ; // 已经存在的条数
				if((adminOrgUnit_el&&adminOrgUnit)||(attencegroupId&&prop_attencegroup)){
		    		that.remoteCall({
						...
						success:function(res){
							...
							$.each(personColl,function(n,value) { 
                                ...
                                var restStartTimeString = ' "restStartTime" : "'+ restStartTime +'"';
                                var restStartTime2String = ' "restStartTime2" : "'+ restStartTime2 +'"';
                                var restEndTimeString = ' "restEndTime" : "'+ restEndTime +'"';
                                var restEndTime2String = ' "restEndTime2" : "'+ restEndTime2 +'"';
					...
					var person_num_split = personNum.split(',');
					for(var i = 0 ; i<person_el_split.length;i++){
							...
							var personString = ' "person" : { "id": "' 
				        							...
				        							+ person_split[i]
				        							+ '"}';		
							var personNumString = ' "person.number" : "'+ person_num_split[i] +'"';
							var startTimeString = ' "startTime" : "'+ startTime +'"';
	...
	,calculateOTtimes : function(rowid){
		...
		if(startTime!="" && endTime!=""){
			...
			var se = endTime.getTime()-startTime.getTime() - (restVal * 1000 * 60); // 毫秒
			var tfl = se/(3600*1000);
			$("#entries").jqGrid('setCell',rowid,"applyOTTime",tfl.toFixed(fixedNum));
		}
	},
	//获取档案历史加班类型的加班补偿方式
