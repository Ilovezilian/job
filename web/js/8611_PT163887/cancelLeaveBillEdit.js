...
shr.defineClass("shr.ats.CancelLeaveBillEdit", shr.framework.Edit, {
	...
	assembleSaveData:function(action)
	{
		...
		var date = model.entries.realLeaBeginTime;
		if(date && personId){
	...
	,getRealLeaveLengthOfDay:function(){
		...
		var isElasticCalLenS;
		try{
			isElasticCalLenS=$("#entries_isElasticCalLen").shrCheckbox("isSelected")
		}catch(err){
		}
		if(!_isHalfDayOff){
			that.remoteCall({
				...
				param:{beginTime:beginTime,
					endTime:endTime,
					leaveBillId:leaveBillId,
					isElasticCalLen:isElasticCalLenS
				},
				success:function(res){
					info =  res;
	...
	addLeaveBillInfoHtml: function (res) {
		...
		var leaveLength = atsMlUtile.numberToUserFormat(res.leaveLength);
		rowBlockHtml += that.addALabelContainerHtml(jsBizMultLan.atsManager_cancelLeaveBillEdit_i18n_27, leaveLength);
		...
		if (($form.valid() && _self.verify())) {
			_self.billCheckAct(function(res){
				...
				if(result==""){
					...
					shr.showConfirm(result+jsBizMultLan.atsManager_cancelLeaveBillEdit_i18n_39,function(){
						try {
		                   parent._submit(false);
		                } catch (e) {
		                    console.info('parent._submit');
		                }
