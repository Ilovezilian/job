shr.defineClass("shr.ats.atttenceEditFormImport", shr.framework.Edit, {
	 ...
	 // 让调用方提供节点 
		...
		shr.ats.atttenceEditFormImport.superClass.initalizeDOM.call(this);
		var that = this;
		lastUipk = this.uipk;
	...
	importAction: function(){
		...
		var callback = function(data){
			if(data && data.length>0){
				...
				if(self.uipk =="com.kingdee.eas.hr.ats.app.AtsLeaveBillAllBatchForm"){//批量请假单导入
					//导入完成之后刷剩余额度和单位
					var allIds = $editGrid.jqGrid('getDataIDs');
					$.each(allIds, function(){ 
						_self.countRemainLimit(this);
						_self.countLeaveLength(this);
						_self.countLeaveTimes(this);
					});					
				}
