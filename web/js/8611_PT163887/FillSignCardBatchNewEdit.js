...
shr.defineClass("shr.ats.FillSignCardBatchNewEdit", shr.ats.atttenceEditFormImport, {
	rowid : "" ,
    cellname : "" ,
    value : "" ,
    iRow : "" ,
	...
	initalizeDOM : function () {
		var _self = this;	
		shr.ats.FillSignCardBatchNewEdit.superClass.initalizeDOM.call(this);
...
//	}
	...
	,assembleModel: function() {
		...
		var model = shr.ats.FillSignCardBatchNewEdit.superClass.assembleModel.call(this);
	...
	,assembleSaveData : function(action){
		var data = shr.ats.FillSignCardBatchNewEdit.superClass.assembleSaveData.call(this,action);
		...
		if(model.entries){
			...
			for( var i = 0 ; i < length ; i++) {
				...
				var date = model.entries[i].attendDate;
				if(date && personId){
					if(i > 0){
						personDateStr +=",";
					}
	...
	,gridCellListener : function(){
		...
		grid.jqGrid("option", {
			  afterEditCell:function (rowid, cellname, value, iRow, iCol) {
					...
					if(cellname=="person"){
			          var hrOrgUnitId=$("#hrOrgUnit_el").val();
			          $(document.getElementById(iRow+"_person")).shrPromptBox().attr("data-params",hrOrgUnitId);
                      $(document.getElementById(iRow+"_person")).shrPromptBox("option", {
                          onchange: function (e, value) {
                              var index = $(this).closest("tr").attr("id")
                              index && $("#entries").jqGrid('setCell', index, "person.number", value.current["person.number"]);
                          }
                      });
					}
			  }
	...
	,getCurrentModel: function(){
		...
		var model = shr.ats.FillSignCardBatchNewEdit.superClass.getCurrentModel.call(this);
		//审核编辑界面重新点击出差类型 josn 转换出错
		if(this.isFromWF())
		{
