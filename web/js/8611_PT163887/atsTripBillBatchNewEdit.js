...
var allDayRemote = 0;
shr.defineClass("shr.ats.atsTripBillBatchNewEdit", shr.ats.atttenceEditFormImport, {
	...
	initalizeDOM : function () {
		var _self = this;
		shr.ats.atsTripBillBatchNewEdit.superClass.initalizeDOM.call(this);
...
//					}
	...
	,initUIFormatter : function(){
		var _self = this;
		var classfullNameService = "com.kingdee.shr.ats.web.formEditImport.AtsTripBillFileEditFormService";
		//界面微调
		...
		$('<button type="button" style="margin-left:4px;margin-right:4px;display: none" class="shrshrbtn-primary shrbtn" id="import">'
			+ jsBizMultLan.atsManager_atsTripBillBatchNewEdit_i18n_2
			+ '</button>').insertAfter($("#addRow_entries"));
		$('.editGrid-toolbar').append('<div style="display: inline;padding-left: 20px;">'
			+ jsBizMultLan.atsManager_atsTripBillBatchNewEdit_i18n_20
			+ '<div>');
		...
		//【BT948778】宽度太长，屏蔽这句
		//$("#entries").setGridWidth($(window).width()*0.88,true);
	}
	...
	,assembleModel: function() {
		...
		var model = shr.ats.atsTripBillBatchNewEdit.superClass.assembleModel.call(this);
...
//			}
			...
			if (lastRowNum<1) return {tripType:defaultTirpType};
				...
				return {tripType:lastTripType,tripReason:lastTripReason,tripStartTime:lastTripStartTime,tripEndTime:lastTripEndTime,tripStartPlace:lastTripStartPlace,tripEndPlace:lastTripEndPlace,remark:lastRemark};
			}
		}	
	}
...
//		var tripBillGrid=waf("#entries");
		tripBillGrid.jqGrid("option", {
			  afterEditCell:function (rowid, cellname, value, iRow, iCol) {
				  ...
				  /*$("#chk_entries_isAllDay_"+rowid).bind("input propertychange", function(){
					  _self.allDayOnChange(rowid);
				  });*/
			  }
...
//	}
	...
	,getCurrentModel : function(){
		...
		var model = shr.ats.atsTripBillBatchNewEdit.superClass.getCurrentModel.call(this);
		//审核编辑界面重新点击出差类型 josn 转换出错
		if(this.isFromWF())
		{
		  var lastRowNum = $('#entries').getGridParam("reccount");
