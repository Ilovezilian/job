...
(function($) {
	...
	aMonths = [$.attendmanageI18n.jqtimeline.month1, 
	           $.attendmanageI18n.jqtimeline.month2, 
	           $.attendmanageI18n.jqtimeline.month3, 
	           $.attendmanageI18n.jqtimeline.month4, 
	           $.attendmanageI18n.jqtimeline.month5, 
	           $.attendmanageI18n.jqtimeline.month6, 
	           $.attendmanageI18n.jqtimeline.month7, 
	           $.attendmanageI18n.jqtimeline.month8, 
	           $.attendmanageI18n.jqtimeline.month9, 
	           $.attendmanageI18n.jqtimeline.month10, 
	           $.attendmanageI18n.jqtimeline.month11, 
	           $.attendmanageI18n.jqtimeline.month12];
	...
	jqTimeLine.prototype._getMonthMarkup = function(num,year){
		...
		if(num== 0){
			retStr='<div class="horizontal-line leftend" style="left:'+_this._current_offset_x+'px">' + 
					...
					'<div class="month">'
				+ $.attendmanageI18n.jqtimeline.month1
				+ '</div></div>';
		}else if(num%2 == 1){
			//retStr = '<div class="horizontal-line month-line odd-month" style="left:'+_this._current_offset_x+'px"></div>';
			// if add year show here , you should expend 'leftend' style from  'horizontal-line leftend'
			...
			$retHtml.mouseover( 
				function(e){
					...
					for (var i = 0; i < neighborEvents.length; i++) {
						...
						var oData = $temp.data('event');//年月日 +  星期 
						//strToolTip = strToolTip + '<div class="msg" id="msg_'+oData.id+'">'+oData.on.toLocaleDateString()+' : '+ oData.name +'</div>';
						var begindate = oData.beginTime;
						begindate = begindate.substr(0,10);//2013-06-08 06:50:00
						var enddate = oData.endTime;
						enddate = enddate.substr(0,10);
						var applydate = oData.applyDate;
						applydate = applydate.substr(0,10);
						
						strToolTip = strToolTip + 
						'<div class="msg" id="msg_'+oData.id+'">' +
						''+begindate 
						+' ' + $.attendmanageI18n.jqtimeline.text1 + ' '
						+ enddate +'   '+oData.typeName+' '+oData.length+'  '+oData.unitType+'  ' +   //开始时间 至 结束时间  年假 2天
						' <br/>' 
						+ $.attendmanageI18n.jqtimeline.text2
						+oData.reason +' '+
						' <br/>' 
						+ $.attendmanageI18n.jqtimeline.text3
						+oData.state +' '+
						' <br/>' 
						+ $.attendmanageI18n.jqtimeline.text4
						+ applydate +' '+
						'</div>';
			...
			$retHtml.click(function(e){
				...
				if($msgs.length == 1){
					...
					var markup =	$('<div class="info">' + 
										...
										'<div>' 
										+ $.attendmanageI18n.jqtimeline.text5
										+ '</div>' + 
