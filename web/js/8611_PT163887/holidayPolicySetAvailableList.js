shr.defineClass("shr.ats.holidayPolicySetAvailableList", shr.ats.AtsAvailableBasicItemList, {
	initalizeDOM : function () {
		shr.ats.holidayPolicySetAvailableList.superClass.initalizeDOM.call(this);
		var that = this;
		$("[title='" + jsBizMultLan.atsManager_holidayPolicySetAvailableList_i18n_5+ "']").parent().remove();
	},
	viewAction: function(billId) {
		
		var billInfoId = $(this.gridId).jqGrid("getCell", billId, "BaseInfo.id");
	
		this.reloadPage({
			uipk: 'com.kingdee.eas.hr.ats.app.HolidayPolicySet.AvailableForm',
			billId: billInfoId,
			orgrefId:billId ,
			relatedFieldId: billInfoId,
	...
	shr.showWarning({message : jsBizMultLan.atsManager_holidayPolicySetAvailableList_i18n_0});
	return;
	}
	...
	var _self = this;
	$('#blindCurrent').attr('values','holiday');
	$('#importDiv').remove();
	var importDiv = $('<div id="importDiv"></div>').appendTo($('body'));	
	importDiv.dialog({
	modal : false,
	title : jsBizMultLan.atsManager_holidayPolicySetAvailableList_i18n_3,
	width : 1035,
	minWidth : 1035,
	height : 505,
	minHeight : 505,
	...
	importDiv.dialog("close");
		...
		if(gridData.length<=0){
			shr.showWarning({message : jsBizMultLan.atsManager_holidayPolicySetAvailableList_i18n_2});
			return;
		}
		...
		openLoader(1,jsBizMultLan.atsManager_holidayPolicySetAvailableList_i18n_4);
		shr.ajax({
			type:"post",
			async:true,
			url:url,
		...
		shr.showWarning({message : jsBizMultLan.atsManager_holidayPolicySetAvailableList_i18n_1});
		return;
		}
		if (selectedIds.length > 0) {
		var bills;
