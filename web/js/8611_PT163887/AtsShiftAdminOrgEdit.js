shr.defineClass("shr.ats.AtsShiftAdminOrgEdit", shr.framework.Edit, {
	  /*是否触发了真正意义上的改变事件*/
	  isTriggerShiftChange : false,
	  firstDefaultShift : null,
	  isClickShiftF7 : false,
      initalizeDOM:function(){
			shr.ats.AtsShiftAdminOrgEdit.superClass.initalizeDOM.call(this);
			var that = this ;
			//初始化分录数据
//			that.initEditGrid("2");
			...
			if(that.getOperateState()=="VIEW"){
				...
				if($("#isElastic").val()==1 && $("#elasticType").val()==1){
					var content = $("#elasticDirection").val()==0
						?shr.formatMsg(jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_84,[atsMlUtile.getFieldOriginalValue("elasticValue")])
						:shr.formatMsg(jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_82,[atsMlUtile.getFieldOriginalValue("elasticValue")]);
			...
			$("#saturday").shrSelect("option", {
		 		onChange : function(e, value) {
		 			var saturday_el=$("#saturday_el").val();
					 if(saturday_el==0){
					 $('#xAtsShift').shrPromptBox('disable');
					 $("#infoss").hide();
					 $('#xitemInfo').empty();
					 $('#xAtsShift').val('');
					  $('#xAtsShift_el').val('');
			  ...
			  $("#infoss").hide();
			   $('#xAtsShift').shrPromptBox('disable');
			   if(that.getOperateState() == "EDIT"){
			   		if(that.uipk=="com.kingdee.eas.hr.ats.app.AtsShiftAdminOrgEdit.form"){
			   		$("#dayType").change(function() {
		 				
		 						var type=$("#dayType_el").val();
		 						if(type != 0){
		 						$('#itemInfo').empty();
								that.initEditGrid("0");
								$("#atsShift_el").val('');
			   ...
			   $("#saturday").on("focus",function(){$(this).blur});
			   	//不可编辑
      	...
      	var that = this;
      	 if(that.getOperateState() == "VIEW" || that.getOperateState() == "EDIT"){
	  		var data = {
					id:$('#form #id').val()||'',
					type:"0",
					uipk:"com.kingdee.eas.hr.ats.app.AtsShiftAdminOrg.form"
			};
			shr.doAjax({
...
//					that.doubleTiggerElastic();
      ...
      ,initEditGrid :function(type){
	  	var row_fields_work =  '<div style="padding-top:15px;" class="row-fluid row-block row_field">'
			...
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_12
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_4
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_3
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_74
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_78
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_30
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_3
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_85
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_86
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_30
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_90
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_88
			+ '</span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_13
			+ '</span></div>'
	  	 	+ '</div>';
		var row_fields_works =  '<div style="padding-top:15px;" class="row-fluid row-block row_field">'
			+ '<div class="spanSelf_01"><span class="cell-RlStdType"></span></div>'
			+ '<div class="spanSelf"><span class="cell-RlStdType">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_12
			+ '</span></div>'
			...
			+ '</span></div>'
	  	 	...
	  	 	$('#xitemInfo').append(row_fields_works);
	  	 	}else{
	  	 	$('#itemInfo').append(row_fields_work);
	  	 	$('#xitemInfo').append(row_fields_works);
	  	 	}
		...
		if(rst != null && rst.records != null && rst.records>0){
	  		...
	  		for(var i=1;i<=rst.rows.length;i++){
				...
				if(that.getOperateState() == "ADDNEW" || that.getOperateState() == "EDIT"){
					...
					$('#xitemInfo').append(row_fields_work);
					}
	...
	,packageF7Value:function(rowNum,segment_value,attendanceType_value,preIsPunchCard_value,nextIsPunchCard_value,preTimeDayType_value,nextTimeDayType_value){	
		   ...
		   segment.data = [{value:"1",alias:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_11},
		                    {value:"2",alias:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_9},
		            		{value:"3",alias:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_10}];
		   ...
		   attendanceType.data = [{value:"1",alias:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_94},
		                    	  {value:"2",alias:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_31},
		            			  {value:"3",alias:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_95},
		            		      {value:"4",alias:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_32}];
		  ...
		  timeDayType.data = [{value:"0",alias:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_35},
		                    	  {value:"1",alias:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_8},
		            			  {value:"2",alias:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_33}];
		   ...
		   preIsPunchCard.data = [{value:"1",alias:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_80},
		                    	  {value:"0",alias:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_29}];
		    ...
		    if(segment_value == null){
    	 	...
    	 	$('#xitemInfo input[name=segment'+rowNum+']').shrSelect(segment);
    	 	...
    	 	$('#xitemInfo input[name=attendanceType'+rowNum+']').shrSelect(attendanceType);
    	 	...
    	 	$('#xitemInfo input[name=preIsPunchCard'+rowNum+']').shrSelect(preIsPunchCard);
    	 	...
    	 	$('#xitemInfo input[name=nextIsPunchCard'+rowNum+']').shrSelect(preIsPunchCard);
    	 	...
    	 	$('#xitemInfo input[name=nextTimeDayType'+rowNum+']').shrSelect(timeDayType);
    	 	...
    	 	$('#xitemInfo input[name=preTimeDayType'+rowNum+']').shrSelect(timeDayType);
...
//						that.doubleTiggerElastic();
	...
	,myExtendValidate:function(){ //扩展自定义校验
	      jQuery.extend(jQuery.validator.messages, {  
    		myTmVldt: jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_36 ,
    		myRestTmVldt: jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_36 ,
    		myRestVldt:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_14,
    		my24Vldt:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_1
		  });  
		  ...
		  }, jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_36);//msg:错误提示文本。已验证
		  ...
		  }, jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_15);//msg:错误提示文本。已验证
	      ...
	      },jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_1);
	}	
	        ...
	        for(var i=0;i<a.length-1;i++){
	           for(var j=i+1;j<a.length;j++){
	              ...
	              if (min1.segment==max1.segment) {
	              	 rt.valid=false;
	                 rt.info=jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_2+getSegmentNameByValue(min1.segment);
	                 return rt;
	              }
	           }
	        }   
	        ...
	        for(var i=0;i<a.length-1;i++){
	           for(var j=i+1;j<a.length;j++){
	              ...
	              if( that.validateTimeLegal(min,max))
	              {
	                 ...
	                 rt.info=jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_97;
	                 return rt;
	              }
	           }
	        }
	...
	,validateItems:function(){
		...
		if(item.length>1 && $('#isElastic').shrCheckbox("isSelected") && $("input#elasticType").shrSelect("getValue").value==0){
    		shr.showWarning({message:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_39});
    	    return false;
    	}
    	...
    	&& $('#isElastic').shrCheckbox("isSelected") && $("input#elasticType").shrSelect("getValue").value==0){
    		shr.showWarning({message:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_40});
    	    return false;
    	}
    	if($('#isElastic').shrCheckbox("isSelected") && $("input#elasticType").shrSelect("getValue").value==1){
			var elasticValueTemp = atsMlUtile.getFieldOriginalValue("elasticValueTemp");
			if(!(parseFloat(elasticValueTemp)>0)){
    			shr.showWarning({message:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_6});
    	        return false;
    		}
    		if(!(/^-?\d+\.?\d{0,2}$/.test(elasticValueTemp))){
    			shr.showWarning({message:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_7});
    	        return false;
    		}
    		if(parseFloat(elasticValueTemp)>=parseFloat(atsMlUtile.getFieldOriginalValue("standardHour"))){
    			shr.showWarning({message:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_83});
    	        return false;
    		}
    		...
    		if ( !(restpretime.trim() == "")){
				...
				if(restpretimeVal>pretimeVal && parseFloat(elasticValueTemp)>=(restpretimeVal-pretimeVal) ){
					shr.showWarning({message:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_96});
    	    		return false;
				}
				if(restpretimeVal<=pretimeVal && parseFloat(elasticValueTemp)>=(24+restpretimeVal-pretimeVal) ){
					shr.showWarning({message:jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_96});
    	    		return false;
				}
			}
	...
	,saveAction: function(event) {
		...
		$("#elasticValue").val(atsMlUtile.getFieldOriginalValue("elasticValueTemp"));
		if(dayType == '0' ){
			...
			var defaultShift_el = $('#atsShift_el').val();
			if(defaultShift_el == null || defaultShift_el == undefined || defaultShift_el.length < 1){
				shr.showWarning({message: jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_37});
				return false;
			}
		}
		else{
	...
	,doSave: function(event, action) {
		...
		if(_self.getOperateState() == "EDIT"){//修改
		shr.doAction({
			target: target,
...
//				localStorage.setItem("atsShiftAdmin_successMsg","保存成功。") ;
			}
		});	
		}else{
		shr.doAction({
			...
			data: data,
			success : function(response) {
				localStorage.setItem("atsShiftAdmin_successMsg",jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_0) ;
				_self.reloadPage({
					uipk: 'com.kingdee.eas.hr.ats.app.AtsShiftAdminOrg.list'
				});
			}
		});
		}
	...
	,assembleSaveData:function(action){
		...
		var adminId=$("#adminOrgUni_el").val()
		model.adminOrgUnit="";
		var items = _self.getItemsInfo();
		var xitems = _self.getXitemsInfo();
		if(items == null){
			return null;
		}
		model.items = items;
	 	...
	 	if(_self.getOperateState() == "EDIT"){
	 		var elasticType=model.elasticType;
	 		if(elasticType == "1"){
	 		model.elasticDirection = $("#elasticDirectionTemp").shrSelect("getValue").value;
	 		model.elasticValue = atsMlUtile.getFieldOriginalValue("elasticValueTemp");
	 		}
	 		adminId=$("#adminOrgUnit_el").val();
	 	}
...
//	 	$('#xitemInfo input[name^=nextFloatAdjusted]').each(function(i,domEle) {
//			  var length = $(domEle).attr("name").substring("nextFloatAdjusted".length);
//	 	      dataArray.push(parseInt(length));
//	 	});
	 	//将数组由小到大排列
...
//	 	});
	 	...
	 	for(var i=0;i<dataArray.length;i++){
	 		var entrie = {
	 			segment: correctValue($('#xitemInfo input[name="segment' + dataArray[i] + '_el"]').val()),
				attendanceType: correctValue($('#xitemInfo input[name="attendanceType' + dataArray[i] + '_el"]').val()),
				preTimeDayType: correctValue($('#xitemInfo input[name="preTimeDayType' + dataArray[i] + '_el"]').val()),
				
				preTime: correctValue($('#xitemInfo input[name="preTime' + dataArray[i] +'"]').val()),
				preIsPunchCard:correctValue($('#xitemInfo input[name="preIsPunchCard' + dataArray[i] +'_el"]').val()),
				preFloatAdjusted:correctValue($('#xitemInfo input[name="preFloatAdjusted' + dataArray[i] +'"]').val()),
				//preUnit:	correctValue($('#itemInfo input[name="preUnit' + dataArray[i] +'_el"]').val()),

				nextTimeDayType:  correctValue($('#xitemInfo input[name="nextTimeDayType' + dataArray[i] +'_el"]').val()),
				nextTime:	correctValue($('#xitemInfo input[name="nextTime' + dataArray[i] +'"]').val()),
				//nextUnit:	correctValue($('#itemInfo input[name="nextUnit' + dataArray[i] +'_el"]').val()),
			
				nextIsPunchCard:	correctValue($('#xitemInfo input[name="nextIsPunchCard' + dataArray[i] +'_el"]').val()),
				nextFloatAdjusted: correctValue($('#xitemInfo input[name="nextFloatAdjusted' + dataArray[i] + '"]').val()),
				segmentInRest:	correctValue($('#xitemInfo input[name="segmentInRest' + dataArray[i] +'"]').val()),
				restPreTime: correctValue($('#xitemInfo input[name="restPreTime' + dataArray[i] + '"]').val()),
				restNextTime: correctValue($('#xitemInfo input[name="restNextTime' + dataArray[i] + '"]').val()),
				
				shiftItemId: correctValue($('#xitemInfo input[name="shiftItemId' + dataArray[i] + '"]').val())
	...
	calSegmentInRestValue : function(restPreTime,restNextTime,preTime,nextTime,predaytype,nextdaytype) {
		...
		if (srartdate.getTime()<predate.getTime() && senddate.getTime()<predate.getTime()) {
			rt.valid = false;
			rt.info = jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_91;
		}
		
		if (srartdate.getTime()<predate.getTime() || srartdate.getTime()>nextdate.getTime()) {
			rt.valid = false;
			rt.info = jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_92;
		}
		if (senddate.getTime()<predate.getTime() || senddate.getTime()>nextdate.getTime()) {
			rt.info = jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_89;
			rt.valid = false;
		}
	...
	valideTimeOfCompare: function(vo){
			...
			if (parseInt(prevalue) == 0 && parseInt(nextvalue) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_76});
				return false
			}
			if (parseInt(prevalue) == 0 && parseInt(nextvalue) == 2) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_77});
				return false;
			}
			if (parseInt(prevalue) == 1 && parseInt(nextvalue) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_73});
				return false;
			}
			if (parseInt(prevalue) == 2) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_75});
				return false;
			}
		}
		//二段班
		if (vo.length == 2) {
			...
			if (offtime1.getTime() < ontime1.getTime()) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_25});
				return false;
			}
			if (offtime2.getTime() < ontime2.getTime()) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_19});
				return false;
			}
			...
			if (parseInt(prevalue1) == 0 && parseInt(nextvalue1) == 2) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_28});
				return false;
			}
			...
			if (parseInt(prevalue1) == 0 && (parseInt(nextvalue2) == 0 || parseInt(nextvalue2) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_24});
				return false;
			}
			//中间组合情况判断
			if (parseInt(nextvalue1) == 1 && parseInt(prevalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_26});
				return false;
			}
			if (parseInt(nextvalue1) == 1 && parseInt(nextvalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_27});
				return false;
			}
			
			//上班时间为当天
			if (parseInt(prevalue1) == 1 && parseInt(nextvalue1) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_23});
				return false;
			}
			if (parseInt(prevalue1) == 1 && parseInt(prevalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_21});
				return false;
			}
			if (parseInt(prevalue1) == 1 && parseInt(nextvalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_22});
				return false;
			}
			//中间组合情况判断
			if (parseInt(nextvalue1) == 1 && parseInt(prevalue2) == 0) {
			...
			if (parseInt(prevalue2) == 2 && (parseInt(nextvalue2) == 0 || parseInt(nextvalue2) == 1) ) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_18});
				return false;
			}
		...
		if (vo.length == 3) {
			...
			if (offtime1.getTime() < ontime1.getTime()) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_68});
				return false;
			}
			if (offtime2.getTime() < ontime2.getTime()) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_44});
				return false;
			}
			if (offtime3.getTime() < ontime3.getTime()) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_52});
				return false;
			}
			
			if (parseInt(prevalue1) == 0 && parseInt(nextvalue1) == 2) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_67});
				return false;
			}
			if (parseInt(prevalue1) == 0 && (parseInt(prevalue2) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_59});
				return false;
			}
			if (parseInt(prevalue1) == 0 && parseInt(nextvalue1) == 1 && (parseInt(prevalue2) == 0 || parseInt(prevalue2) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_60});
				return false;
			}
			//---
			if (parseInt(prevalue1) == 0 && (parseInt(nextvalue2) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_61});
				return false;
			}
			if (parseInt(prevalue1) == 0 && parseInt(nextvalue1) == 1 && (parseInt(nextvalue2) == 0 || parseInt(nextvalue2) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_62});
				return false;
			}
			...
			if (parseInt(prevalue1) == 0 && (parseInt(prevalue3) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_63});
				return false;
			}
			if (parseInt(prevalue1) == 0 && parseInt(nextvalue1) == 1 && (parseInt(prevalue3) == 0 || parseInt(prevalue3) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_64});
				return false;
			}
			...
			if (parseInt(prevalue1) == 0 && (parseInt(nextvalue3) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_65});
				return false;
			}
			if (parseInt(prevalue1) == 0 && parseInt(nextvalue1) == 1 && (parseInt(nextvalue3) == 0 || parseInt(nextvalue3) == 2)) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_66});
				return false;
			}
			...
			//上班时间为当天
			if (parseInt(prevalue1) == 1 && parseInt(nextvalue1) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_58});
				return false;
			}
			if (parseInt(prevalue1) == 1 && parseInt(prevalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_54});
				return false;
			}
			if (parseInt(prevalue1) == 1 && parseInt(nextvalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_55});
				return false;
			}
			if (parseInt(prevalue1) == 1 && parseInt(prevalue3) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_56});
				return false;
			}
			if (parseInt(prevalue1) == 1 && parseInt(nextvalue3) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_57});
				return false;
			}
			//中间组合情况判断
			if (parseInt(nextvalue1) == 1 && parseInt(prevalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_69});
				return false;
			}
			if (parseInt(nextvalue1) == 1 && parseInt(nextvalue2) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_70});
				return false;
			}
			if (parseInt(nextvalue1) == 1 && parseInt(prevalue3) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_71});
				return false;
			}
			if (parseInt(nextvalue1) == 1 && parseInt(nextvalue3) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_72});
				return false;
			}
			
			if (parseInt(prevalue2) == 2 && (parseInt(nextvalue2) == 0 || parseInt(nextvalue2) == 1) ) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_43});
				return false;
			}
			
			if (parseInt(nextvalue2) == 1 && parseInt(prevalue3) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_45});
				return false;
			}
			if (parseInt(nextvalue2) == 1 && parseInt(nextvalue3) == 0) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_46});
				return false;
			}
			
			if (parseInt(nextvalue2) == 2 && (parseInt(prevalue3) == 0 || parseInt(prevalue3) ==1) ) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_47});
				return false;
			}
			if (parseInt(nextvalue2) == 2 && (parseInt(nextvalue3) == 0 || parseInt(nextvalue3) == 1) ) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_48});
				return false;
			}
			
			if (parseInt(prevalue3) == 2 && (parseInt(nextvalue3) == 0 || parseInt(nextvalue3) == 1) ) {
				shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_51});
				return false;
			}
	...
	valideTimeOfCompareNew: function(vo){
			...
			if (newVo.length == 1) {
				...
				if(temp.preDateTime.getTime()>temp.nextDateTime.getTime()){
				  shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_93});
				  return false;
				}
				//加上不能大于24小时的判断
				if(temp.nextDateTime.getTime()-temp.preDateTime.getTime()>1*24*60*60*1000){
				  shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_97});
				  return false;
				}
			}
			...
			if (newVo.length == 2) {
				...
				if(preDateTime1.getTime()>nextDateTime1.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_20});
				   return false;
				}
				
				if(preDateTime2.getTime()<nextDateTime1.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_17});
				   return false;
				}
				
				if(preDateTime2.getTime()>nextDateTime2.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_16});
				   return false;
				}
				//加上不能大于24小时的判断
		  ...
		  if (newVo.length == 3) {
			   ...
			   if(preDateTime1.getTime()>nextDateTime1.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_53});
				   return false;
				}
				
				if(preDateTime2.getTime()<nextDateTime1.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_42});
				   return false;
				}
				
				if(preDateTime2.getTime()>nextDateTime2.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_41});
				   return false;
				}
				
				if(preDateTime3.getTime()<nextDateTime2.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_50});
				   return false;
				}
				
				if(preDateTime3.getTime()>nextDateTime3.getTime()){
				   shr.showWarning({"message" : jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_49});
				   return false;
				}
	...
	,addElasticDoc: function() {
		...
		$("#elasticType_down").find("a").eq(0).attr("title",jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_38);
		$("#elasticType_down").find("a").eq(1).attr("title",jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_79);
	}
	,addElasticHtml:function(){
		...
		+'<div class="field_label" style="padding-right: 5px;padding-left: 0;" title="'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_81
			+ '">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_81
			+ '</div>'
		+'</div>'
		...
		+'<div class="field_label" style="padding-right: 0px;padding-left: 0;" title="'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_5
			+ '">'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_5
			+ '</div>'
		+'</div>'
		...
		+'<div class="field_label" title="'
			+ jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_87
	...
	verify: function() {
		...
		var regEx = new RegExp("\\-","gi"); //i不区分大小写 g匹配所有
	 	var tripStartTime = atsMlUtile.getFieldOriginalValue("startTime");
		var tripEndTime = atsMlUtile.getFieldOriginalValue("endTime");
		...
		var tripStartTimeOfDate = new Date(tripStartTime); 
	 	...
	 	if (longTime < 0) {
	 		shr.showInfo({message: jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_34});
			return false;
	 	}
		}
	 	return true;
...
function getSegmentNameByValue(val){
   if("1" == val){
     return jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_11;
   }else if("2" == val){
     return jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_9;
   }else if("3" == val){
     return jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_10;
   }
}
...
function getDayTypeValueByName(typeName){
   if(jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_35 == typeName){
        return "0";
   }else if(jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_8 == typeName){
   	    return "1";
   }else if(jsBizMultLan.atsManager_AtsShiftAdminOrgEdit_i18n_33 == typeName){
   	    return "2"
   }else{
        return "";
   }
